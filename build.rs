...
fn main() {
    ...
    // was built for so `runtime_objects.rs` can name `artifacts-rwk/<triple>.a`
...
/// Embeds the AOT runtime archive (`rts-runtime-rwk`, over `rts-core-rwk` +
/// `rts-std-rwk` + `rts-node-rwk`) into the binary, compressed.
///
/// The `_rwk` in the output names distinguished it from the old engine's
/// `runtime_support.*`, which is no longer produced. Left as it is: the names
/// are internal to `OUT_DIR` and renaming them buys nothing.
///
/// It never fails the build: the JIT paths (`rts run`, `rts test`) never touch
/// this archive, so a missing `rts-runtime-rwk`
