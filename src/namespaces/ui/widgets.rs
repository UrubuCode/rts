use fltk::{button, enums::Color, frame, input, misc, output, prelude::*, valuator};
...
pub extern "C" fn __RTS_FN_NS_UI_WIDGET_RESIZE(handle: u64, x: i64, y: i64, w: i64, h: i64) {
    apply_widget_op(handle, |wid| {
        wid.resize(x as i32, y as i32, w as i32, h as i32)
    });
}
...
pub extern "C" fn __RTS_FN_NS_UI_CHECK_VALUE(handle: u64) -> i8 {
    with_entry(handle, |e| {
        if let UiEntry::CheckButton(b) = e {
            if b.value() { 1 } else { 0 }
        } else {
            0
        }
...
pub extern "C" fn __RTS_FN_NS_UI_RADIO_VALUE(handle: u64) -> i8 {
    with_entry(handle, |e| {
        if let UiEntry::RadioButton(b) = e {
            if b.value() { 1 } else { 0 }
        } else {
            0
        }
...
pub extern "C" fn __RTS_FN_NS_UI_OUTPUT_SET_VALUE(handle: u64, ptr: *const u8, len: i64) {
    let text = str_from_abi(ptr, len).to_owned();
    with_entry_mut(handle, |e| {
        if let UiEntry::Output(o) = e {
            let _ = o.set_value(&text);
        }
    });
}
