...
fn f64_args(args: &[JsValue]) -> (f64, f64) {
...
fn i64_args(args: &[JsValue]) -> (i64, i64) {
...
pub fn dispatch(callee: &str, args: &[JsValue]) -> Option<DispatchOutcome> {
    match callee {
        ...
        "rts.hotops.i64_sub" => {
            ...
            Some(DispatchOutcome::Value(JsValue::Number(a.wrapping_sub(b) as f64)))
        }
        "rts.hotops.i64_div" => {
            let (a, b) = i64_args(args);
            if b == 0 {
            ...
            Some(DispatchOutcome::Value(JsValue::Number((a / b) as f64)))
        }
        "rts.hotops.i64_mod" => {
            let (a, b) = i64_args(args);
            if b == 0 {
        ...
        "rts.hotops.i64_eq" => {
            let (a, b) = i64_args(args);
            Some(DispatchOutcome::Value(JsValue::Bool(a == b)))
        }
        "rts.hotops.i64_lt" => {
            let (a, b) = i64_args(args);
            Some(DispatchOutcome::Value(JsValue::Bool(a < b)))
        ...
        "rts.hotops.i64_to_string" => {
            ...
            Some(DispatchOutcome::Value(JsValue::String(i64_to_string_fast(n))))
        }
        "rts.hotops.f64_to_string" => {
            ...
            let s = if n.is_nan() {
                ...
                if n.is_sign_negative() { "-Infinity".to_string() } else { "Infinity".to_string() }
            } else if n.fract() == 0.0 && n >= i64::MIN as f64 && n <= i64::MAX as f64 {
                i64_to_string_fast(n as i64)
            } else {
                n.to_string()
            };
            Some(DispatchOutcome::Value(JsValue::String(s)))
        }
        _ => None,
    }
}
