...
/// Os alvos. Uma página por linha: nome do ficheiro, host, caminho.
///
/// O critério de entrada é UM: HTML servido PRONTO. Uma página que monta o DOM
/// em JavaScript mede o vazio — o nosso lado não corre `<script>` e o extrator
/// do Chrome desliga-o de propósito, portanto uma SPA compararia duas páginas
/// vazias e chamaria 100% à coincidência.
const ALVOS: string[][] = [
  ["wiki-fisica", "pt.wikipedia.org", "/wiki/F%C3%ADsica"],
  ["wiki-rust-en", "en.wikipedia.org", "/wiki/Rust_(programming_language)"],
  ["python-docs", "docs.python.org", "/3/tutorial/introduction.html"],
  ["hn", "news.ycombinator.com", "/"],
  ["example", "example.com", "/"],
];

for (let a = 0; a < ALVOS.length; a = a + 1) {
  const nome = ALVOS[a][0];
  const ALVO_HOST = ALVOS[a][1];
  const html = buscar(ALVO_HOST, ALVOS[a][2]);
  ...
  for (;;) {
    ...
    else if (alvo.indexOf("/") === 0) { alvo = "https://" + ALVO_HOST + alvo; }
