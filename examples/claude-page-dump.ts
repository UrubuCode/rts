...
const ALVO_HOST = "pt.wikipedia.org";
const ALVO_PATH = "/wiki/Brasil";
const html = buscar(ALVO_HOST, ALVO_PATH);
writeFileSync("pagina.html", html);
let css = "";
let i = 0;
for (;;) {
  ...
  // uma folha pode vir como `//host/x` ou `/x` — resolver contra o alvo é o
  // que um browser faz, e sem isso a página real perde o CSS todo.
  // o `href` vem do HTML CRU, onde `&` está escrito `&amp;` — pedir o URL sem
  // desfazer isso devolve outra coisa (ou nada) do servidor.
  ...
  css = css + "\n" + buscar(resto.substring(0, barra), resto.substring(barra));
}
writeFileSync("pagina.css", css);
console.log("wa.html:", html.length, "| wa.css:", css.length);
