// Música na engine de síntese TS — "Ode à Alegria" (Beethoven), polifônica.
//
// IMPORTANTE: todo o áudio é gerado num ÚNICO loop INLINE no top-level. O RTS
// tem um limite de codegen (issue aberta): gerar áudio via funções aninhadas
// que fazem I/O (audio.write/available_frames) com estado global faz o stream
// rodar free-running (ring nunca acumula → áudio picotado). Inline no top-level
// funciona em tempo-real perfeito. Por isso o sequenciador é uma TABELA de
// eventos processada dentro do mesmo loop, sem funções de render/play.
//
// Rodar: target/release/rts.exe run examples/music.ts
...
if (stream === 0) {
  ...
  io.print("♪ Ode à Alegria — engine TS inline — " + sr + "Hz " + ch + "ch");

  // ── vozes (arrays paralelos) ──
  const MAX = 8;
  ...
  // ── sequência de notas (MIDI da melodia + baixo) ──
  // Ode à Alegria: E E F G | G F E D | C C D E | E. D D (x2)
  ...
  // ── loop principal INLINE: percorre as notas, e dentro de cada nota roda o
  //    loop de áudio que renderiza+escreve respeitando backpressure. ──
  let seqIdx = 0;
  while (seqIdx < NOTES) {
    // dispara a nota: melodia (sine) + baixo (saw) em slots livres
    const melHz = 440.0 * math.pow(2.0, (seqMel[seqIdx] - 69.0) / 12.0);
    // acha slot livre p/ melodia
    let mslot = -1;
    ...
    // toca a duração da nota (com release: solta 60ms antes do fim)
  ...
  // deixa o release final decair
  let tail = 0;
  const tailFrames = (sr * 500) / 1000;
  while (tail < tailFrames) {
    const free = audio.available_frames(stream);
    if (free <= 0) { time.sleep_ms(2); continue; }
    let n = free;
    if (n > 1024) n = 1024;
    if (n > tailFrames - tail) n = tailFrames - tail;
    let s = 0;
    for (let f = 0; f < n; f++) {
      let mix = 0.0;
      for (let i = 0; i < MAX; i++) {
        if (vStage[i] !== 0) {
          if (vStage[i] === 4) { vEnv[i] = vEnv[i] - rRate; if (vEnv[i] <= 0.0) { vEnv[i] = 0.0; vStage[i] = 0; } }
          let osc = 0.0;
          if (vWave[i] === 0) osc = math.sin(vPhase[i] * 2.0 * math.PI);
          else osc = 2.0 * vPhase[i] - 1.0;
          mix = mix + osc * vAmp[i] * vEnv[i];
