...
```
[▰▰▰▰▰▰▰▰▰▰▰▰▰▰▰▰▰▱▱▱] 84.2%   1276/1515 fixtures passing
```

| Metric | Value |
|---|---|
| **Parity** | **84.2%** (1276/1515) |
| ✅ RTS = Bun = Node | 1276 |
| ❌ RTS diverges | 178 |
| 💥 RTS runtime error | 61 |
| 🛠️  **Left to fix** | **239** |
| ⚠️ Bun ≠ Node (skip) | 0 |
| 🚫 Rejected (RTS-only) | 0 |
| 📦 Total fixtures | 1516 |

_Updated: 2026-09-17 — [how to add a fixture](the spec removed 2026-08-03 (see git history))_
...
**DOM engine state** (`crates/rts-dom/PLAN.md` §0): **68/96 lots done**, 8 partial, pending: Q, U, V–Y, BT-1, BT-2, BT-3, BT-4, BT-5, FM, PCT, REGUA, TEXTO, SC, BR, DISP, LOG, IFC, INTR, USED, borda-conflito-hidden. The paint ruler (pixels against Blink, `scripts/css_pintura.md`) needs a browser and runs locally; its last number is recorded there.

*Updated 2026-09-16 by CI (`dom-rulers`).*
<!-- CSS_DOM_STATS_END -->
...
End-to-end process time (includes startup/JIT compile), median of 20 runs after 3 warmups, GitHub Actions `windows-latest` — commit `2f7e3c1`.

| Bench | Bun | Node | Deno | RTS JIT | **RTS AOT** | AOT vs Bun | AOT vs Node |
|---|---|---|---|---|---|---:|---:|

_Updated: 2026-09-16 — run locally with `powershell -File bench/benchmark.ps1`_
