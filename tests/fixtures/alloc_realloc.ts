...
if (p2 == 0) {
  io.print("FAIL: realloc retornou 0");
} else {
  const v1 = ptr.read_i64(p2);
  const v2 = ptr.read_i64(ptr.offset(p2, 8));
  const h1 = gc.string_from_i64(v1); io.print(h1); gc.string_free(h1); // 12345
  const h2 = gc.string_from_i64(v2); io.print(h2); gc.string_free(h2); // 67890
