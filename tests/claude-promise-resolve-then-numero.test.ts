...
//
// `promise.wait` (drain síncrono da superfície antiga) virou `await` de topo:
// o mesmo ponto de observação — o valor DEPOIS do `.then` — na superfície padrão.

const inteiro = await Promise.resolve(7).then(function (v) { return v; });
const negativo = await Promise.resolve(-42).then(function (v) { return v; });
const zero = await Promise.resolve(0).then(function (v) { return v; });
// o valor tem de ser USÁVEL, não só transportado
const dobrado = await Promise.resolve(5).then(function (v) { return v * 2; });
const encadeado = await Promise.resolve(3)
...
const doAwait = await viaAwait().then(function (v) { return v; });
