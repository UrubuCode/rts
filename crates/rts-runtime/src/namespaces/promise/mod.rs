...
//! `docs/specs/rts-core-engine.md`). `then`/`catch`/`finally` perderam o sufixo
...
/// Promise<T> async com state machine + waiters via tokio oneshot. Base para async/await (issue #412 / epic #411).
#[rts_namespace(promise)]
impl PromiseNs {
    ...
    #[rts_fn]
    pub fn new_resolved(value: I64) -> Handle {
    ...
    #[rts_fn]
    pub fn new_rejected(error: I64) -> Handle {
    ...
    #[rts_fn]
    pub fn resolve(promise: U64, value: I64) -> I64 {
    ...
    #[rts_fn]
    pub fn reject(promise: U64, error: I64) -> I64 {
        with_slot(promise, 0, |slot| {
    ...
    #[rts_fn]
    pub fn try_value(promise: U64) -> I64 {
        with_slot(promise, 0, |slot| {
    ...
    #[rts_fn(ts = "then(p: number, fn: (v: number) => number): number")]
    pub fn then(p_handle: U64, fp: U64) -> Handle {
    ...
    #[rts_fn(ts = "catch(p: number, fn: (e: number) => number): number")]
    pub fn catch(p_handle: U64, fp: U64) -> Handle {
        let slot_arc = with_entry(p_handle, |entry| match entry {
            Some(Entry::PromiseAsync(arc)) => Some(arc.clone()),
            _ => None,
        });
    ...
    #[rts_fn(ts = "finally(p: number, fn: () => void): number")]
    pub fn finally(p_handle: U64, fp: U64) -> Handle {
        let slot_arc = with_entry(p_handle, |entry| match entry {
            Some(Entry::PromiseAsync(arc)) => Some(arc.clone()),
            _ => None,
        });
    ...
    #[rts_fn]
    pub fn all(promises: U64) -> Handle {
    ...
    #[rts_fn]
    pub fn race(promises: U64) -> Handle {
        let handles = collect_promise_handles(promises);
        let slots = collect_slots(&handles);
        let result = promise_slot::new_pending();
        let result_handle = alloc_entry(Entry::PromiseAsync(result.clone()));
    ...
    #[rts_alias(of = all_settled)]
    pub fn allSettled(promises: U64) -> Handle {
        unreachable!()
    }
    ...
    #[rts_fn]
    pub fn any(promises: U64) -> Handle {
        let handles = collect_promise_handles(promises);
        let slots = collect_slots(&handles);
        let result = promise_slot::new_pending();
        let result_handle = alloc_entry(Entry::PromiseAsync(result.clone()));
    ...
    #[rts_fn(ts = "create(fn: any, args?: number): number")]
    pub fn create(fn_handle: U64, args_vec_handle: U64) -> Handle {
        let result = promise_slot::new_pending();
        let result_clone = result.clone();
    ...
    #[rts_fn]
    pub fn all_settled(promises: U64) -> Handle {
        let handles = collect_promise_handles(promises);
        let slots = collect_slots(&handles);
        let result = promise_slot::new_pending();
        let result_handle = alloc_entry(Entry::PromiseAsync(result.clone()));
