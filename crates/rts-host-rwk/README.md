# rts-host-rwk — where a compiler and a runtime meet
...
and no language. `rts-core-rwk` implements what the language calls out for and
...
if it is what an operation does at run time, it belongs in `rts-core-rwk`.
...
- **The runtime symbols.** `rts-codegen`'s `RuntimeOp` states the names it
  emits calls to; `rts-core-rwk` exports names derived from its Rust function
