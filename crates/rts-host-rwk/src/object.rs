...
//! first allocation. See `rts-runtime-rwk/src/aot.rs` for the write, and for why
...
/// defined by `rts-runtime-rwk`'s `main` — see this module's own doc comment.
...
pub struct ObjectProgram {
    ...
    /// all left for a linker to resolve against the `rts-runtime-rwk` archive.
    pub bytes: Vec<u8>,
...
pub fn compile_to_object(source: &str) -> Result<ObjectProgram, HostError> {
    ...
    // `Region` — see `rts_core_rwk::heap::STRIDE`'s own doc comment.
    let bases = RegionBases::single(
        RegionBase::Symbol(REGION_BASE_SYMBOL.to_owned()),
        rts_core_rwk::heap::STRIDE,
    );
...
/// `rts-runtime-rwk/tests` reading a manifest this function wrote, not by a
/// shared derive.
