//! The six binary/text codecs `Buffer` needs, moved here from `rts-node-rwk`.
//!
//! # Why moved rather than copied
//!
//! A `Buffer` instance method — `toString`, `write`, `fill` with a string
//! pattern — needs exactly these codecs, and `rts-node-rwk`'s copy was written
...
//! `crates/rts-host-rwk/tests/node_modules.rs`'s
