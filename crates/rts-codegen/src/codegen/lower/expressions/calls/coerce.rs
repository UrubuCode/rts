...
pub(super) fn lower_coerce_to_boolean(ctx: &mut FnCtx, call: &CallExpr) -> Result<Option<TypedVal>> {
    ...
    if let Some(arg) = call.args.first() {
        ...
        // (#784) `null` literal e `undefined` ident sao falsy direto — antes
        // de lower_expr (que materializaria "undefined" como string handle).
        if matches!(arg.expr.as_ref(), swc_ecma_ast::Expr::Lit(swc_ecma_ast::Lit::Null(_))) {
        ...
        if matches!(tv.ty, ValTy::Handle) {
            // Caminho generico: handle 0 (null em RTS) ou string_len <= 0
            // (handle invalido retorna -1) ambos => false; len > 0 => true.
            ...
            let result = ctx.builder.ins().icmp(IntCC::SignedGreaterThan, len, zero);
