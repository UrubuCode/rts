...
pub(crate) fn lower_new(ctx: &mut FnCtx, new_expr: &swc_ecma_ast::NewExpr) -> Result<TypedVal> {
    ...
    // (#218) `new Proxy(target, handler)` DRENADO: a classe "Proxy" é registrada
    // no Registry com construtor → `__RTS_FN_GL_PROXY_NEW`, então o dispatch
    // genérico de construtor acima (`global_class_lookup("Proxy")`) constrói o
    // Entry::Proxy sem braço hardcoded. As traps seguem em runtime via
    // resolve_proxy nos caminhos MAP_*/INVOKE.
