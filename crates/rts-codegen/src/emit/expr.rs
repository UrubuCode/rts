...
pub fn emit_expr(
    ...
    match &expr.kind {
        ...
        ExprKind::Template { parts, expressions } => {
            emit_template(builder, scope, ctx, parts, expressions)
        }
...
fn emit_template(
    ...
    scope: &mut Scope,
    ctx: &mut Ctx,
    parts: &[crate::syntax::TemplatePart],
    expressions: &[Expr],
) -> EmitResult<ValueId> {
...
fn cooked(part: &crate::syntax::TemplatePart) -> EmitResult<&str> {
    match &part.cooked {
        Some(text) => Ok(text),
...
/// are one string, exactly as two occurrences of one literal are.
