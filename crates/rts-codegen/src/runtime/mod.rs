...
pub enum RuntimeOp {
    ...
    /// `+x`, and the `ToNumeric` read of `x++`/`x--`.
    ///
    /// They were `x * 1` through [`RuntimeOp::Multiply`], the same observable
    /// steps for every value — until `rts`'s `operators.mul` let an object
    /// answer `*`, and `+v` would have called it with `1`. A call because
