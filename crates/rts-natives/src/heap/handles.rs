...
pub enum Entry {
    ...
    Rtse {
        ...
        /// Optional collector hook for handles the AUTHORED struct (`data`) itself
        /// owns in its own fields — e.g. a Rust `Map`/`Set` whose keys/values are
        /// caller-supplied words. `None` (the overwhelmingly common case: `alloc_rtse`)
        /// means `data` carries no reachable handle, matching today's behaviour
        /// exactly. `Some` is set only by [`alloc_rtse_traced`]; `trace_children`
        /// calls it with `&*data` alongside the existing `props` walk. See
        /// [`RtseTrace`] for why this is a static fn pointer, not a trait object.
        trace: Option<RtseTraceFn>,
    },
    ...
    fn trace_children(&self, visit: &mut dyn FnMut(u64)) {
        match self {
            ...
            Entry::Rtse {
                ...
                // (RtseTrace) The authored struct itself may own handles in its OWN
                // fields (e.g. a Rust `Map`/`Set` storing caller words) — `data` is
                // opaque `dyn Any` to the collector, so a class opts in by declaring
                // `#[rtse::trace]` fields, which the macro compiles into this fn
                // pointer. `None` (`alloc_rtse`, the common case) skips this — same
                // behaviour as before this arm existed.
...
/// A `#[rtse::class]` struct that owns heap handles in its OWN fields (not via
/// `props`, the ad-hoc JS property bag) — e.g. a Rust-backed `Map`/`Set` whose
/// keys/values are caller-supplied words. The collector cannot see into an
/// opaque `Box<dyn Any>`, so a class that needs this implements `trace_handles`
/// (generated by `#[rtse::trace]` on its fields — see `rts-macro`'s
/// `class::mod::gen_struct`) and allocates via [`alloc_rtse_traced`] instead of
/// [`alloc_rtse`].
pub trait RtseTrace {
    /// Call `visit` once per value WORD this struct owns. A word may be a raw
    /// handle or a boxed PolyValue; the collector's marker decides what it means.
    /// Never call `visit` with `0` (the null/no-handle sentinel).
    fn trace_handles(&self, visit: &mut dyn FnMut(u64));
}

/// Erased form of [`RtseTrace::trace_handles`] stored in `Entry::Rtse::trace`.
/// A plain `fn` pointer (not a `Box<dyn Fn>` or vtable) because the concrete `T`
/// is known at the `alloc_rtse_traced` call site and a non-capturing closure
/// coerces to `fn` for free — no allocation, no dynamic dispatch overhead beyond
/// the downcast every `Entry::Rtse` access already pays.
pub type RtseTraceFn = fn(&(dyn std::any::Any + Send + Sync), &mut dyn FnMut(u64));

/// Like [`alloc_rtse`], but for a struct that declares its own reachable handles
/// via `T: RtseTrace` (see that trait's doc). The collector calls
/// `T::trace_handles` alongside the ordinary `props` walk in `trace_children`.
pub fn alloc_rtse_traced<T: std::any::Any + Send + Sync + RtseTrace>(
    class: &'static str,
    v: T,
) -> u64 {
    fn trace_erased<T: std::any::Any + RtseTrace>(
        data: &(dyn std::any::Any + Send + Sync),
        visit: &mut dyn FnMut(u64),
    ) {
        if let Some(t) = data.downcast_ref::<T>() {
            t.trace_handles(visit);
        }
    }
    alloc_entry(Entry::Rtse {
        class,
        data: Box::new(v),
        props: Default::default(),
        trace: Some(trace_erased::<T> as RtseTraceFn),
    })
}

/// Composes a value word into an [`RtseTrace`]-declared field: a raw handle
/// (`u64`/`i64`, visited iff non-zero — the same "0 = no handle" convention
/// `trace_children`'s other arms use) or a container of them
/// (`Option`/`Vec`/`IndexMap` values). Lets `#[rtse::trace]`'s generated impl
/// call one uniform method per field without the macro needing to know the
/// field's concrete type.
pub trait TraceWord {
    fn trace_word(&self, visit: &mut dyn FnMut(u64));
}

impl TraceWord for u64 {
    fn trace_word(&self, visit: &mut dyn FnMut(u64)) {
        if *self != 0 {
            visit(*self);
        }
...
impl<T: TraceWord> TraceWord for Option<T> {
    fn trace_word(&self, visit: &mut dyn FnMut(u64)) {
        if let Some(v) = self {
            v.trace_word(visit);
        }
...
impl<T: TraceWord> TraceWord for Vec<T> {
    fn trace_word(&self, visit: &mut dyn FnMut(u64)) {
        for v in self {
            v.trace_word(visit);
        }
...
impl<K, V: TraceWord, S> TraceWord for indexmap::IndexMap<K, V, S> {
    fn trace_word(&self, visit: &mut dyn FnMut(u64)) {
...
mod tests {
    ...
    /// A `#[rtse::class]` struct that owns a handle in one of its OWN fields (not
    /// `props`) — e.g. what a Rust-backed `Map`/`Set` needs for its stored keys/
    /// values. Without `RtseTrace` + `alloc_rtse_traced`, `data` is opaque `dyn
    /// Any` to the collector and this handle would be swept while live, exactly
    /// like `props` was before `rtse_props_are_traced` guarded it.
    #[test]
    fn rtse_traced_struct_fields_are_traced() {
        use crate::Traceable;

        struct Probe {
            held: u64,
        }
        impl RtseTrace for Probe {
            fn trace_handles(&self, visit: &mut dyn FnMut(u64)) {
                self.held.trace_word(visit);
            }
        }

        let child = alloc_entry(Entry::String(b"reachable-only-via-struct-field".to_vec()));
        let h = alloc_rtse_traced("Probe", Probe { held: child });

        let mut visited = Vec::new();
        with_entry(h, |e| {
            e.unwrap().trace_children(&mut |v| visited.push(v));
        });

        assert!(
            visited.contains(&child),
            "alloc_rtse_traced's struct-owned handle must be visited; visited={visited:?}"
        );
        free_handle(child);
        free_handle(h);
    }
