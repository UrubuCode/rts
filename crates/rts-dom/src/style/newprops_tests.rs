...
fn filter_e_clip_path_chegam_crus_ao_paint() {
    ...
    assert_eq!(c.clip_path.as_deref(), Some("polygon(50% 0%, 100% 100%, 0% 100%)"));
    ...
    assert_eq!(parse_inline("-webkit-filter: invert(1)").filter.as_deref(), Some("invert(1)"));
    assert_eq!(
        parse_inline("-webkit-clip-path: inset(10px)").clip_path.as_deref(),
        Some("inset(10px)")
    );
}
...
fn grid_line_aceita_as_quatro_formas_que_o_corpus_escreve() {
    ...
    assert_eq!(parse_inline("grid-column-start: auto").grid_column_start, Some(GridLine::Auto));
    assert_eq!(parse_inline("grid-column-start: 7").grid_column_start, Some(GridLine::Line(7)));
    assert_eq!(parse_inline("grid-column-end: -1").grid_column_end, Some(GridLine::Line(-1)));
    assert_eq!(parse_inline("grid-row-end: span 2").grid_row_end, Some(GridLine::Span(2)));
}
...
fn linha_zero_nao_e_uma_linha_de_grid() {
    ...
    assert_eq!(parse_inline("grid-column-end: span 0").grid_column_end, None);
    ...
    assert_eq!(parse_inline("grid-row: span 1 / span 1").grid_row_start, Some(GridLine::Span(1)));
}
...
fn grid_column_nao_declarado_computa_auto_e_nao_polui_o_style_inline() {
    ...
    assert_eq!(s.get_property("grid-column"), "", "el.style só tem o declarado");
...
fn background_clip_guarda_as_quatro_caixas_incluindo_text() {
    ...
    assert_eq!(parse_inline("background-clip: text").background_clip, Some(BackgroundClip::Text));
    ...
    assert_eq!(parse_inline("color: red").computed_value("background-clip", None), "border-box");
}
...
fn os_dois_blend_mode_partilham_o_vocabulario_mas_nao_o_campo() {
    ...
    assert_eq!(parse_inline("mix-blend-mode: luminosity").mix_blend_mode, Some(BlendMode::Luminosity));
...
fn text_shadow_reusa_a_sombra_de_caixa_mas_sem_spread() {
    ...
    let s = parse_inline("text-shadow: 1px 2px 3px 4px red").text_shadow.unwrap();
    ...
    let n = parse_inline("text-shadow: 0 1px 2px rgba(0,0,0,0.5)").text_shadow.unwrap();
...
fn text_shadow_computa_com_a_cor_a_frente_como_o_chrome() {
    ...
    assert_eq!(parse_inline("color: red").computed_value("text-shadow", None), "none");
}
...
fn a_cauda_de_pintura_nao_muda_um_pixel_hoje() {
    ...
    let sem = layout("<div style='width:50px;height:10px;background:#ff0000'>a</div>", 800.0);
    let com = layout(
