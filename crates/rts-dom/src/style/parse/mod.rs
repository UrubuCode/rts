...
/// lógica `-inline-`/`-block-` — `style::logical::e_eixo_dependente`) contra
/// um contexto VAZIO: sem árvore, `parse_inline` não tem custom props nem
/// `direction`/`writing-mode` herdados nenhuns para lhes dar, e é isso — não
/// "fica por resolver" — que um bloco solto, sem ancestral, responde
/// (caem no inicial `ltr`/`horizontal-tb`, como sempre respondeu antes deste
/// lote). É a mesma pergunta que `dom::cascade::computed_style_idx_inner`
/// faz por elemento, só que aqui o "elemento" não existe.
...
pub(crate) fn apply_specified_declaration(
    ...
    // logical `-inline-`/`-block-` (margin/padding/border/inset/inline-size,
    // `style::logical::e_eixo_dependente`) adia pela MESMA razão de fundo —
    // o lado físico depende de `direction`/`writing-mode`, que também só se
    // conhecem por elemento, nunca no momento em que esta regra é compilada
    // (`style::logical`, cabeçalho "Quando isto resolve"). A mesma fila, os
    // dois motivos — `css_var_refs` só conta o motivo que já tinha nome.
    if val.contains("var(") {
        crate::bump!(css_var_refs);
        block
            .pending
