...
//! calls `apply_relative_offset` on its own box once it has positioned it.
//!
...
//! reuses `layout/positioned/relative.rs::apply_relative_offset` rather than writing a
...
use crate::layout::{LayoutCtx, apply_relative_offset};
...
/// `(0, 0)` — `apply_relative_offset` itself already skips the walk in that
/// last case.
...
fn aplica_offset(
    ...
    apply_relative_offset(caixa, css, avail_w, avail_h, font_size, desde, ctx, list);
}
