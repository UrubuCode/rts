...
/// SUPERSEDIDO pelo lote `flex-writing-mode`: o corte (`dom::
/// direction_herdada`, removido) assumia que "a ordem dos itens da linha
/// ainda não inverte por `direction`" — verdade quando `flex-reverse-order`
/// o escreveu, falso desde que este lote acrescentou exatamente essa
/// inversão a `layout::flex::layout_children_horizontal`
/// (`eixos_flex::reverse_efetivo`, sem precisar de `row-reverse`). Com a
/// ORDEM já correta, a exceção passou a pôr a margem no lado físico OPOSTO
/// ao dos itens (re-medido em `gap-003-rtl`/`gap-006-rtl` depois do merge) —
/// exatamente o "margem certa, ordem errada" que o corte dizia evitar,
/// agora ao contrário. `margin-inline-end` resolve NORMAL numa linha, como
/// já resolvia numa coluna (`logicas_inline_end_numa_coluna_de_flex_rtl_resolve_normal`
/// abaixo, que continua a valer, sem exceção nenhuma a distingui-las).
#[test]
fn logicas_inline_end_numa_linha_de_flex_rtl_resolve_normal() {
    const HTML: &str = r#"<style>
  body { direction: rtl; }
  .f { display: flex; }
...
<div class="f"><div id="alvo"></div></div>"#;
    ...
    assert_eq!(css.margin.left, crate::style::Side::Len(crate::style::Dimension::Px(10.0)), "inline-end em rtl e a esquerda, como numa coluna");
    assert_eq!(css.margin.right, crate::style::Side::Unset);
}
