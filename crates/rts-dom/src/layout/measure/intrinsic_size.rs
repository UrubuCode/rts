...
//! and `measure::intrinsic_content_width` (max-content width), each its own
...
//! pass gives — the same machinery [`super::measure::child_outer_height`]
...
//! size from a max-content one (`crate::layout::flex::column_shrink::min_main` already says as
...
///   for `Min`, exactly
///   [`measure::intrinsic_content_width`](super::measure::intrinsic_content_width)
...
pub(in crate::layout) fn intrinsic_size(
    ...
    match axis {
        Axis::Inline => Some(match kind {
            ...
            IntrinsicKind::Max => super::measure::intrinsic_content_width(dom, id, font, ctx),
        }),
        Axis::Block => {
            let w = inline_size?;
...
<div id="f"><div style="width:50px"></div><div style="width:70px"></div></div>"#;
        ...
        let id = id_de(&dom, "#f");
        let c = ctx();
        let direto = super::super::measure::intrinsic_content_width(&dom, id, 16.0, &c);
        let via_eixo =
