...
pub(in crate::layout) fn layout_children_column_wrap(
    ...
    // é a mesma pergunta que `column_rtl::cross_x` já resolve para a
    ...
    for (col, cw) in columns.into_iter().zip(col_cross.into_iter()) {
        ...
        for (j, it) in positions.iter().enumerate() {
            ...
            if it.is_text {
                ...
                // `column_rtl::cross_x`): a mesma pergunta do caminho de UMA
                ...
                let child_x = super::column_rtl::cross_x(
                    css.direction,
                    css.writing_mode,
                    x,
                    cw,
...
//   (`column_rtl::cross_x`, PASSO 5, que continua a fazer a sua parte). Achado
