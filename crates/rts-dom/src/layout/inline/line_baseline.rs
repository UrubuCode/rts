...
use super::vertical_align::{Envelope, envelope_com_baseline, topo_do_item_com_baseline};
use super::*;
...
pub(in crate::layout) fn envelope_da_linha(
    ...
    let proprias: Vec<(f32, f32, VerticalAlign)> = line
        ...
        .filter_map(|s| super::run_font::do_segmento(dom, &s.owners, family, font_size, ctx.measurer))
...
fn atomo(dom: &Dom, seg: &Segment, content_w: f32, ctx: &LayoutCtx) -> Option<Atomo> {
    ...
    match kind {
        ...
        AtomicKind::Widget => Some(Atomo {
            altura,
            ascent: super::line_inline_block::ascent_do_item(dom, id, altura, content_w, ctx),
            valign: dom
...
fn ascent_do_bloco(
    ...
    if flex {
        ...
        let dentro = super::line_inline_block::ascent_do_item(dom, id, borda, content_w, ctx);
...
fn ascent_do_gerado(caixa: &crate::pseudo::PseudoBox, altura: f32, content_w: f32, ctx: &LayoutCtx) -> f32 {
    ...
    let arestas = crate::layout::block::pseudo_box::resolve_arestas(css, &r);
