...
pub(in crate::layout) fn emitir_atomo(
    ...
    line_advance: f32,
    at_linha: usize,
    superficies: &mut super::inline_fragments::Superficies,
    text_top: f32,
    ascent: f32,
    font_size: f32,
    family: Option<&str>,
    ...
    let (desde, (rx, ry)) = (list.pieces.len(), crate::layout::positioned::relative::offset_do_inline(dom, seg.owners.last().copied(), ctx));
    match kind {
        AtomicKind::Widget => {
        ...
        AtomicKind::Replaced => {
            ...
            if super::line::e_canvas(dom, a_idx) {
        ...
        AtomicKind::Block => {
            ...
            let topo = match envelope {
                Some(env) => super::line_baseline::topo_do_atomo(dom, seg, cy, env, font_size, family, content_w, ctx),
                None => cy,
            };
            layout_block(
                dom,
        ...
        AtomicKind::Gerada(pe, ParteGerada::Atomo) => {
            ...
            let topo = envelope
                .as_ref()
                .map_or(cy, |env| super::line_baseline::topo_do_atomo(dom, seg, cy, env, font_size, family, content_w, ctx));
    ...
    crate::layout::positioned::relative::desloca_desde(list, desde, kind.tem_corpo().then_some(caixa), rx, ry);
    ...
    let ja_registado = matches!(
        ...
        && (dom.image_dims(a_idx).is_some() || super::line::e_canvas(dom, a_idx)));
    if !ja_registado {
