...
/// `occupied_spans` são os intervalos `[início, fim)` de trilhas que cada item cobre.
pub(in crate::layout) fn collapsed(
    collapsible: &[bool],
    n: usize,
    occupied_spans: impl Iterator<Item = (usize, usize)>,
) -> Vec<bool> {
    let mut occupied = vec![false; n];
    for (a, b) in occupied_spans {
        for o in occupied.iter_mut().take(b.min(n)).skip(a) {
            *o = true;
        }
    }
    (0..n)
        .map(|i| collapsible.get(i).copied().unwrap_or(false) && !occupied[i])
        .collect()
}
...
pub(in crate::layout) fn as_fixed_zero(tracks: &mut [GridTrack], collapsed: &[bool]) {
    for (t, &c) in tracks.iter_mut().zip(collapsed) {
        if c {
...
pub(in crate::layout) fn visible_count(n: usize, collapsed: &[bool]) -> usize {
    (0..n).filter(|&i| !collapsed.get(i).copied().unwrap_or(false)).count()
}

/// A soma das calhas de um eixo: só as que separam duas trilhas visíveis.
pub(in crate::layout) fn gutters(n: usize, collapsed: &[bool], gap: f32) -> f32 {
    visible_count(n, collapsed).saturating_sub(1) as f32 * gap
}

/// O início de cada trilha (e o fim da última, no índice `n`), com a
/// distribuição de conteúdo já aplicada. `distribution` é `None` quando o
...
pub(in crate::layout) fn starts(
    sizes: &[f32],
    collapsed: &[bool],
    origin: f32,
    gap: f32,
    distribution: Option<(crate::style::JustifyContent, f32)>,
) -> Vec<f32> {
    let n = sizes.len();
    let used = sizes.iter().sum::<f32>() + gutters(n, collapsed, gap);
    let (initial, between) = match distribution {
        Some((v, container)) => {
            let free = (container - used).max(0.0);
            crate::layout::flex::column::justify_offsets(v, free, visible_count(n, collapsed))
        }
        None => (0.0, 0.0),
    };
    let mut out = Vec::with_capacity(n + 1);
    let mut cursor = origin + initial;
