...
pub(in crate::layout) fn intrinsic_content_width(
    ...
    if let Some(w) = dom.computed_style_idx(id).and_then(|css| super::input::tamanho_natural_controlo(dom, id, &css, ctx)).map(|(w, _)| w) { dom.intrinsic_width_put(key, w); return w; }
