...
| U-pintura-1 | rotação/skew EXATOS + recorte de `overflow` na régua de pintura | 5 | ◐ em curso: `PushTransform`/`PopTransform` na `DisplayList` (matriz em vez do `rect` mutado por aproximação), quadrilátero real no raster e no egui (`Shape::Mesh`); `filhos_antes` do `BeginClip` de overflow estava errado (contava DEPOIS de o filho já existir — nunca recortava nada); `overflow-x`/`overflow-y` mistos computam o `visible` sozinho como `auto` (CSS Overflow 1 §3). `claude-transform-origin` 6,06%→0%, `claude-overflow` 5,57%→0%, `claude-sel-target` 2,55%→0,05% (meta `fixar-hash` lido no raster) | `feat/dom-lote-pintura-rotacao-clip` (2026-09-04) | `scripts/css_pintura.md` |
