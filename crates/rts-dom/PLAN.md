...
| S-decor | decoração/imagens/host — texto-decoração, `text-shadow`, `cursor`/`pointer-events`, `background-image` em camadas, `clip-path` | 4 | ◐ em curso — computado FECHADO nas 5 réguas medidas: shorthand `text-decoration` preenche `-style`/`-thickness`; `text-shadow` com N sombras; `cursor`/`pointer-events` já estavam (só faltava o hit-test respeitar `none`); `background-image` em camadas + `-repeat`/`-position`/`-size` seguindo o nº de camadas; `clip-path` tinha campo e NUNCA tinha leitor. Pintura: só o hit-test (`pointer-events: none` transparente ao clique). Sem pintura: sombra de texto, linha de decoração por cor/espessura, gradiente/url em camadas, recorte de `clip-path`. `cursor: url(...)` fica RELATIVO (nenhuma propriedade desta engine resolve URL contra uma base) | `feat/dom-lote-decoracao-host` (2026-09-04) | `claude-text-decoration-longhands`, `claude-text-shadow`, `claude-cursor-pointer-events`, `claude-background-camadas`, `claude-clip-path-inset` (computado dos 5 fecha em `style::tests::computed_corpus`; corpus/suite por medir pelo orquestrador) |
| T | fontes reais | 3 | ☐ | — | §5 |
