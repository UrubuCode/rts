...
| flex-baseline | `align-items`/`align-self: baseline` no flex; `flex-wrap` de 3 estados | 7 | ☑ `AlignItems::Baseline` resolvida POR LINHA (`layout/flex_baseline.rs`, módulo novo — `flex.rs` já estava no tecto de 500): ascent do item = `margin-top` + `linha_ib::ascent_do_item` (agora `pub(in crate::layout)`, reusada em vez de duplicada); o de maior ascent fica no início da linha, os outros descem `max_ascent - ascent_i`. `FlexWrap` (NoWrap/Wrap/WrapReverse) substitui o `Option<bool>` que fazia `"wrap-reverse"` cair em `Some(false)`, idêntico a `nowrap`; `WrapReverse` agrupa como `Wrap` e só inverte a ORDEM das linhas no eixo cruzado (§8.3). CORTES ditos: baseline só na LINHA (coluna cai em `FlexStart`, o fallback que a spec já prevê); pseudo-item e texto solto fora do grupo; só `first baseline`; `align-content` não reespelha físicos sobre a ordem já invertida; `flex-direction:column`+`wrap` continua sem multi-coluna (corte pré-existente, fora deste lote). `cargo test --lib --features metrics`: 904, 0 falhas, filtro `flex_baseline` 4/4 à primeira. Corpus/WPT/suite: medidos no fecho | `feat/dom-lote-flex-baseline` → `bf59f4574`+`22a6bec40` (2026-09-04) | `claude-flex-align-baseline`, `claude-flex-wrap-reverse-linha`, `claude-flex-wrap-reverse` |
