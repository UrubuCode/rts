...
| I | folha de UA em CSS real | 2 | ◐ em curso — `style/ua.css`+`style/ua.rs` integrados na cascade (origem UA); `used_display` e o recuo de lista já não usam `block/ua.rs`; parte 4 (`scrollbar.rs`) só desenhada, não feita | `feat/dom-lote-i-folha-ua` | `<th>` negrito/centro; `scrollbar.rs` apagado |
| J | `DeclarationRecord` e `revert` | 2 | ◐ em curso — `revert`/`revert-layer` implementados (marcador por nome + resolução condicional em `style/stylesheet/revert.rs`, não um `DeclarationRecord` materializado por propriedade — ver o ficheiro para o porquê); por medir pelo orquestrador | `feat/dom-lote-j-declaration-record` | fixtures `claude-revert`/`claude-revert-layer` (sem `.esperado.json` — medir no Chrome); `cargo test -p rts-dom style::stylesheet::revert` |
