...
| L | cache de fragmentos em flex/grid/tabela | 2 | ◐ em curso — `FragmentKey` ganhou `forced_outer_w/h`+`shrink_to_fit`; flex-row, flex-column e grid chamam `layout_block_reusing` para os ITENS; containers com filhos de tamanho imposto NUNCA são costurados (guard novo em `costurar`); tabela e out-of-flow não tocados; `cargo check -p rts-dom --tests` limpo; `dom_metrics` por medir pelo orquestrador | `feat/dom-lote-l-cache-flex-grid` (2026-09-04) | `dom_metrics`: subárvores reusadas numa app flex |
