...
| inline-flex | `display:inline-flex` é flex por DENTRO e inline-level por FORA | 7 | ☑ `DisplayKind::InlineFlex` — `to_display_code` devolve o MESMO código de `Flex` (2, horizontal) para o eixo dos FILHOS, e entra nos seis sítios que já tratavam `InlineBlock` como inline-level/atómico (`is_inline_level`, `is_inline_block` em `caixa.rs`, a 5.ª cópia em `vertical.rs`, os dois `matches!` de `establishes_block_formatting_context` e o de `margin_child_role`, todos em `bloco.rs`) — sem isso um `inline-flex` sem `width` nunca encolhia (tomava a largura do bloco) e nunca ficava na linha do pai (um bloco por linha). `getComputedStyle().display` responde `inline-flex` (`style/fmt_values.rs`, braço exaustivo — o compilador forçou). CORTE dito: `flex-wrap:wrap` num `inline-flex` não quebra em várias linhas (sem `InlineFlexWrap`; `effective_display` não sintetiza um código WRAP para ele) — a baseline de um inline-flex MULTI-LINHA fica por resolver. Corpus/WPT/suite: medidos no fecho | `feat/dom-lote-inline-flex` → `72f0105c7` (2026-09-04) | `claude-flex-inline-flex-inline-level`, `claude-inline-flex-outer-display` |
