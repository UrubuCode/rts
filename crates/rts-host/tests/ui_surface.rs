...
use rts_std::test::{record, reset};

/// Roda um módulo e responde os testes que reportaram divergência.
fn failures(source: &str) -> Vec<String> {
    reset();
    let mut program = rts_host::compile(source).expect("um módulo que compila");
    program.run();
    record()
        .into_iter()
...
fn os_dois_especificadores_resolvem_e_seus_membros_sao_chamaveis() {
    // O que o `rts-std` recusou registrar até agora: um `import` de
...
fn um_objeto_de_opcoes_incompleto_desenha_com_os_defaults_em_vez_de_nada() {
    // A decisão de aridade de `rts-ui::value`, exercida: `drawRect` tinha
