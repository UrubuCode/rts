...
const win = openWindow("rts-ui — motor novo", 720, 420, 0);
if (win > 0) {
    let frame = 0;
    while (frame < 600 && isOpen(win)) {
        ...
        drawText(win, { x: 40, y: 100, text: "rts:egui + rts:input, via rts-ui",
...
"#;
    ...
    let mut program = match rts_host::compile(source) {
        Ok(program) => program,
        Err(error) => {
