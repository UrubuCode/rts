...
fn preserved(func: &Function) -> Vec<ValueId> {
    plan_suspension(func).spill.slots().iter().map(|s| s.value).collect()
}
...
fn everything_live_across_a_suspension_is_preserved_not_only_references() {
    let types = TypeRegistry::new();
    let mut func =
        suspending(&[Repr::F64, Repr::Ref(RefKind::Bytes)], &[Repr::F64, Repr::Tagged]);
    ...
    assert_eq!(kept.len(), 2, "and asking a client to tell them apart is asking it to err");
}
...
fn a_value_nothing_reads_after_a_suspension_is_not_preserved() {
    ...
    let entry = func.entry;
    let mut b = FuncBuilder::new(&mut func, &types, entry);
    let _ = b.arith(NumOp::Add, unused_after, unused_after).expect("proven");
    b.suspend();
    b.ret(&[]);

    assert!(preserved(&func).is_empty(), "the record holds what resuming needs, nothing else");
}
...
fn resume_labels_number_the_points_in_program_order() {
    ...
    assert_eq!(labels, vec![0, 1, 2], "a resumption is a jump selected by a number");
}
...
fn a_suspension_is_a_point_the_collector_must_be_able_to_read() {
    ...
    assert!(frame.resume_label.is_some(), "and it is also where control comes back");
}
...
fn an_allocation_and_a_suspension_are_both_described_but_only_one_resumes() {
    ...
    assert!(frames[0].resume_label.is_none(), "an allocation is not a resumption target");
...
fn parking_a_frame_that_did_not_declare_it_may_is_rejected() {
    let types = TypeRegistry::new();
    let mut func =
        Function::new(Signature { params: vec![], returns: vec![], may_suspend: false });
    ...
    assert!(
        errors.iter().any(|e| matches!(e, VerifyError::UndeclaredSuspension { .. })),
