...
pub(super) fn value(
    context: &mut rts_core_rwk::entry::Context,
    held: Vec<(String, Vec<Interface>)>,
) -> u64 {
    let root = rts_core_rwk::entry::make_object(context);
        ...
        let entries = addresses
            ...
            .map(|entry| {
                ...
                for (key, text) in [
                    ...
                    rts_core_rwk::entry::put_member(context, object, key, value);
                }
                let internal = rts_core_rwk::entry::boolean_value(entry.internal);
                rts_core_rwk::entry::put_member(context, object, "internal", internal);
                ...
                let cidr = match &entry.cidr {
                    Some(text) => rts_core_rwk::entry::make_string(context, text),
                    None => rts_core_rwk::entry::null_in(context),
                };
                rts_core_rwk::entry::put_member(context, object, "cidr", cidr);
                if let Some(scope) = entry.scopeid {
                    let value = rts_core_rwk::entry::make_number(f64::from(scope));
                    rts_core_rwk::entry::put_member(context, object, "scopeid", value);
                }
                object
            })
            .collect();
        let array = rts_core_rwk::entry::make_array_in(context, entries);
        rts_core_rwk::entry::put_member(context, root, &name, array);
    }
    root
}
