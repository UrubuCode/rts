...
pub(super) fn object(context: &mut Context) -> u64 {
    ...
    for (name, entries) in [
        ...
        let group = rts_core_rwk::entry::make_object(context);
        for (key, value) in entries {
            let number = rts_core_rwk::entry::make_number(f64::from(*value));
            rts_core_rwk::entry::put_member(context, group, key, number);
        }
        rts_core_rwk::entry::put_member(context, root, name, group);
    }
    ...
    #[cfg(windows)]
    {
        let errno = rts_core_rwk::entry::get_member(context, root, "errno");
        for (key, value) in WINSOCK {
            let number = rts_core_rwk::entry::make_number(f64::from(*value));
            rts_core_rwk::entry::put_member(context, errno, key, number);
        }
    }
    root
}
