...
//! `rts-core-rwk::entry` (its `Context`/table inventory has no process
//! table; `entry::modules` is exactly the value API every other
//! `rts-node-rwk` module already builds on, so this reuses it rather than
...
pub fn namespace(context: &mut Context) -> u64 {
    ...
    // methods HERE, once — [`rts_core_rwk::entry::make_prototype`] records a
    ...
    let child_process_class = rts_core_rwk::entry::make_prototype(context, "ChildProcess", spawn_async::METHODS);
    rts_core_rwk::entry::put_member(context, namespace, "ChildProcess", child_process_class);
    namespace
}
