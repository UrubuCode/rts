...
pub(super) fn install(context: &mut Context, namespace: u64) {
    ...
    let placeholder = rts_core_rwk::entry::make_object(context);
...
pub(super) extern "C" fn construct(
    ...
    let (instance, label, trigger) = rts_core_rwk::entry::with_runtime(|context| {
        let prototype = rts_core_rwk::entry::make_prototype(context, "AsyncResource", METHODS);
        ...
        let label = rts_core_rwk::entry::string_in(context, type_label).unwrap_or_default();
        let given = match rts_core_rwk::entry::is_object(context, options) {
            true => rts_core_rwk::entry::get_member(context, options, "triggerAsyncId"),
            false => options,
        };
        let trigger = rts_core_rwk::entry::number_of(given)
            .filter(|held| *held >= 0.0)
            .map_or(inherited, |held| held as u64);
        let stored = rts_core_rwk::entry::make_number(id as f64);
        rts_core_rwk::entry::put_member(context, instance, "__asyncId__", stored);
        let stored = rts_core_rwk::entry::make_number(trigger as f64);
        rts_core_rwk::entry::put_member(context, instance, "__triggerId__", stored);
        let stored = rts_core_rwk::entry::make_string(context, &label);
        rts_core_rwk::entry::put_member(context, instance, "__type__", stored);
...
fn id_on(instance: u64, field: &str) -> u64 {
    rts_core_rwk::entry::with_runtime(|context| {
        let held = rts_core_rwk::entry::get_member(context, instance, field);
        rts_core_rwk::entry::number_of(held).map_or(0, |value| value as u64)
    })
}
...
/// observe an exception, because [`rts_core_rwk::entry::call`] cannot unwind
/// into one.
extern "C" fn run_in_async_scope(
    ...
    let result =
        rts_core_rwk::entry::call(body, this_arg, first, undefined, undefined, undefined);
...
extern "C" fn emit_destroy(_e: u64, this: u64, _a: u64, _b: u64, _c: u64, _d: u64) -> u64 {
    let id = id_on(this, "__asyncId__");
    let already = rts_core_rwk::entry::with_runtime(|context| {
        let held = rts_core_rwk::entry::get_member(context, this, "__destroyed__");
        let already = rts_core_rwk::entry::number_of(held).is_some();
        if !already {
            let mark = rts_core_rwk::entry::make_number(1.0);
            rts_core_rwk::entry::put_member(context, this, "__destroyed__", mark);
        }
        already
    });
    match already {
...
extern "C" fn async_id(_e: u64, this: u64, _a: u64, _b: u64, _c: u64, _d: u64) -> u64 {
    rts_core_rwk::entry::make_number(id_on(this, "__asyncId__") as f64)
}
...
extern "C" fn trigger_async_id_method(
    ...
    rts_core_rwk::entry::make_number(id_on(this, "__triggerId__") as f64)
}
...
pub(super) extern "C" fn execution_async_id(
    ...
    rts_core_rwk::entry::make_number(current_id() as f64)
}
...
pub(super) extern "C" fn trigger_async_id(
    ...
    rts_core_rwk::entry::make_number(trigger as f64)
}
...
pub(super) extern "C" fn execution_async_resource(
    ...
    match entered {
        ...
        None => match TOP_LEVEL.with(Cell::get) {
            0 => rts_core_rwk::entry::undefined_value(),
