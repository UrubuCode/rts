...
pub(super) extern "C" fn create_hook(_e: u64, _this: u64, options: u64, _b: u64, _c: u64, _d: u64) -> u64 {
    ...
    let hook = rts_core_rwk::entry::with_runtime(|context| {
        let prototype = rts_core_rwk::entry::make_prototype(context, "AsyncHook", METHODS);
        let hook = rts_core_rwk::entry::make_instance(context, prototype);
        let absent = rts_core_rwk::entry::undefined_in(context);
        for field in FIELDS {
            let given = rts_core_rwk::entry::get_member(context, options, field);
            if given == absent {
                continue;
            }
            match rts_core_rwk::entry::is_callable_in(context, given) {
                true => rts_core_rwk::entry::put_member(context, hook, field, given),
...
fn callbacks(field: &str) -> Vec<u64> {
    ...
    rts_core_rwk::entry::with_runtime(|context| {
        ...
        for hook in hooks {
            let callback = rts_core_rwk::entry::get_member(context, hook, field);
            if rts_core_rwk::entry::is_callable_in(context, callback) {
                found.push(callback);
            }
        }
        found
...
pub(super) fn fire_init(async_id: u64, type_label: &str, trigger: u64, resource: u64) {
    ...
    let (label, undefined) = rts_core_rwk::entry::with_runtime(|context| {
        (
            rts_core_rwk::entry::make_string(context, type_label),
            rts_core_rwk::entry::undefined_in(context),
        )
    });
    let id = rts_core_rwk::entry::make_number(async_id as f64);
    let trigger = rts_core_rwk::entry::make_number(trigger as f64);
    for callback in found {
        rts_core_rwk::entry::call(callback, undefined, id, label, trigger, resource);
    }
}
...
pub(super) fn fire_id(field: &str, async_id: u64) {
    ...
    let id = rts_core_rwk::entry::make_number(async_id as f64);
    for callback in found {
        rts_core_rwk::entry::call(callback, undefined, id, undefined, undefined, undefined);
    }
}
