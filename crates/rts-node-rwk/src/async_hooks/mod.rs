...
//! of something. Searched `rts_core_rwk::entry` (`entry/mod.rs`'s `Context`
...
//! [`rts_core_rwk::entry::call`]. They are built.
//!
...
//! [`rts_core_rwk::entry::make_callable`] takes a bare `extern "C" fn` with no
...
//! properties with strings ([`rts_core_rwk::entry::put_member`]) and mints no
...
//! ([`rts_core_rwk::entry::throw`] ends the process), so both report to stderr
...
fn attach(
    ...
    let prototype = rts_core_rwk::entry::make_prototype(context, name, methods);
    let constructor = rts_core_rwk::entry::get_member(context, namespace, name);
