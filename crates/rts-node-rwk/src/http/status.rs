...
pub(super) fn status_codes_object(context: &mut rts_core_rwk::entry::Context) -> u64 {
    let object = rts_core_rwk::entry::make_object(context);
    for (code, phrase) in CODES {
        let value = rts_core_rwk::entry::make_string(context, phrase);
        rts_core_rwk::entry::put_member(context, object, &code.to_string(), value);
    }
    object
}

pub(super) fn methods_array(context: &mut rts_core_rwk::entry::Context) -> u64 {
    let values: Vec<u64> = METHODS.iter().map(|m| rts_core_rwk::entry::make_string(context, m)).collect();
