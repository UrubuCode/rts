...
//!
//! - `rts-host-rwk`'s `graph.rs` walks a program's whole relative-import graph
//!   and emits every file into ONE compilation, so each module of the program
//!   is in `rts-core-rwk`'s specifier table at run time, keyed by its resolved
//!   absolute path — `entry::module_at_name` reads it, and `rts-std-rwk`'s
