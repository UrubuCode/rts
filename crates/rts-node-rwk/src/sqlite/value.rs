...
//! by [`rts_core_rwk::entry::make_buffer`] over a copy of the bytes (the same
...
//! in `rts_core_rwk::entry::modules` — the only value API this crate is
//! allowed to reach — and [`rts_core_rwk::entry::bigint_new`] is not part of
...
//! range or not — becomes a JS `number` through [`rts_core_rwk::entry::make_number`].
...
//! throw either — `rts_core_rwk::entry::throw` ends the PROGRAM (see its own
...
//!
//! [`with_runtime`]: rts_core_rwk::entry::with_runtime
