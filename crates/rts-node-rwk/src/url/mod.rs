...
//!
//! `rts-cranelift` and `rts-core-rwk`'s `entry` module have no URL parser, no
...
//!
//! `rts_core_rwk::entry::accessor` (`get x()`/`set x(v)`) is reachable only
//! from `#[rtse::class]`-declared classes with compile-time-interned keys —
//! `rts-node-rwk` is restricted to `rts_core_rwk::entry::modules`, which
...
//!   `rts_core_rwk::entry::modules` function raises a catchable exception —
//!   `rts-core-rwk::entry::throw` ends the whole program rather than
