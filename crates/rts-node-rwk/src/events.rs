...
//! [`rts_core_rwk::entry::make_prototype`] builds it under the name
...
//! [`rts_core_rwk::entry::make_instance`] then gives each `new EventEmitter()`
...
//! exposes (`rts_core_rwk::entry` has `settled`/`drain_microtasks` for
...
//! [`rts_core_rwk::entry::make_callable`] hands back a fixed function
...
pub fn namespace(context: &mut Context) -> u64 {
    ...
    let prototype = rts_core_rwk::entry::make_prototype(context, "EventEmitter", METHODS);
    let constructor = rts_core_rwk::entry::get_member(context, namespace, "EventEmitter");
...
extern "C" fn make_emitter(_e: u64, this: u64, _a: u64, _b: u64, _c: u64, _d: u64) -> u64 {
    rts_core_rwk::entry::with_runtime(|context| {
        ...
        let emitter = match rts_core_rwk::entry::is_object(context, this) {
            true => this,
...
extern "C" fn remove_all_listeners(_e: u64, this: u64, event: u64, _b: u64, _c: u64, _d: u64) -> u64 {
    ...
    if event == absent {
        let names: Vec<u64> = collect_array(rts_core_rwk::entry::own_keys(events_object(this)));
        for key in names {
...
extern "C" fn emit(_e: u64, this: u64, event: u64, a0: u64, a1: u64, a2: u64) -> u64 {
    ...
    if wrappers.is_empty() {
        if rts_core_rwk::entry::text_of(event).as_deref() == Some("error") {
...
fn crash_on_unhandled_error(error: u64) -> ! {
    match rts_core_rwk::entry::described(error) {
...
extern "C" fn listeners(_e: u64, this: u64, event: u64, _b: u64, _c: u64, _d: u64) -> u64 {
    ...
    rts_core_rwk::entry::make_array(functions)
}
...
extern "C" fn get_max_listeners(_e: u64, this: u64, _a: u64, _b: u64, _c: u64, _d: u64) -> u64 {
    let absent = rts_core_rwk::entry::undefined_value();
    let stored = rts_core_rwk::entry::with_runtime(|context| {
        rts_core_rwk::entry::get_member(context, this, "__maxListeners__")
    });
    if stored == absent {
        rts_core_rwk::entry::make_number(DEFAULT_MAX_LISTENERS.load(Ordering::Relaxed) as f64)
    } else {
        stored
    }
}
...
extern "C" fn static_set_max_listeners(_e: u64, _this: u64, n: u64, target: u64, _c: u64, _d: u64) -> u64 {
    let absent = rts_core_rwk::entry::undefined_value();
    if target == absent {
        if let Some(value) = rts_core_rwk::entry::number_of(n) {
...
fn emit_meta(this: u64, meta: &str, event: u64, listener: u64) {
    ...
    let absent = rts_core_rwk::entry::undefined_value();
    for wrapper in wrappers {
        rts_core_rwk::entry::call(wrapper_fn(wrapper), this, event, listener, absent, absent);
    }
}
