...
//! there. What IS already vetted for `rts-node-rwk` is `libc` (§2 of that
//! doc: "libc-as-FFI-declaration is accepted... only a C compile/vendor step
//! is rejected"), and the raw-`extern "system"` FFI pattern
//! `crates/rts-node-rwk/src/os/*.rs` and `crates/rts-node-rwk/src/tty/
...
//! Windows (same shape as `crates/rts-node-rwk/src/os/cpus.rs`'s existing
