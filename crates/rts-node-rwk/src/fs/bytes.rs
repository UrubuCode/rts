...
//! onto real bytes, over `rts_core_rwk::entry::{bytes_of, write_bytes,
//! make_bytes}`.
//!
...
pub(super) fn option_number(options: u64, name: &str) -> Option<f64> {
    ...
    let value = with_runtime(|context| rts_core_rwk::entry::get_member(context, options, name));
...
fn array_items(array: u64) -> Vec<u64> {
    ...
    let length = with_runtime(|context| {
        let value = rts_core_rwk::entry::get_member(context, array, "length");
        rts_core_rwk::entry::number_of(value)
    })
