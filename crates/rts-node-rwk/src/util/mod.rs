...
//!
//! Searched `rts-core-rwk`'s host surface next (`entry/modules.rs` and every
...
//! not used. `TextEncoder`/`TextDecoder` live in `rts-std-rwk`'s globals and are
...
//!
//! - **`TextEncoder` / `TextDecoder`.** They exist — `rts-std-rwk`'s
