...
//!
//! # Why this is a SECOND record, not the one `rts-std-rwk::test` already has
//!
//! `crates/rts-std-rwk/src/test.rs` already records `describe`/`test`/`expect`
//! results and `rts-host-rwk` already reads that record — the obvious question
...
//! is consistent with the architecture fact both docs restate: `rts-node-rwk`
//! is independent of `rts-std-rwk`/`rts-runtime` and must not gain a
//! dependency on either to get a native piece.
//!
//! `rts-std-rwk::test::record`/`reset` ARE already `pub fn`s — a public
//! accessor is not the blocker. A crate dependency is: `rts-node-rwk` has no
//! `rts-std-rwk` line in its `Cargo.toml` (not a file this task owns, and not
...
thread_local! {
    /// Thread-local for the same reason `rts-std-rwk::test`'s is: a context
