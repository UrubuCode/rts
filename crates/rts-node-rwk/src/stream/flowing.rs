...
//! - **The loop that turns it**: [`rts_core_rwk::entry::declare_loop_source`]
...
//! `rts-host-rwk`'s `run.rs`) and which `promise_await` runs while parked.
//!
...
///
/// `@@`-prefixed, which is not decoration: `rts-core-rwk`'s `symbol.rs` filters
