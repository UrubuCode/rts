...
pub(super) extern "C" fn connect(_e: u64, this: u64, a: u64, b: u64, c: u64, _d: u64) -> u64 {
    ...
    registry::with_sockets(|table| {
        table.insert(id, SocketEntry { instance: this, queue: Default::default(), stream: None, closed: false });
    });
...
pub(super) fn adopt(stream: TcpStream, remote: String) -> u64 {
    ...
    registry::with_sockets(|table| {
        table.insert(id, SocketEntry { instance, queue: Default::default(), stream: Some(stream), closed: false });
    });
    if let Some(reader) = reader {
        spawn_reader(id, reader);
    }
