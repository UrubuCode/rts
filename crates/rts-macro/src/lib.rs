...
/// with the ABI shape DERIVED FROM THE RUST SIGNATURE.
///
...
/// stripped, upper-cased, suffixed `_ABI`. The function itself passes through
/// untouched — this attribute only ADDS a sibling item.
///
...
#[proc_macro_attribute]
pub fn abi(_a: TokenStream, item: TokenStream) -> TokenStream {
    let func = syn::parse_macro_input!(item as syn::ItemFn);
    let sym = func.sig.ident.to_string();
...
fn abi_const_name(sym: &str) -> String {
    let base = sym
...
/// Map a Rust parameter/return type to its `AbiType` token, or `None` when the
/// type has no single-slot ABI spelling (pointers — see the `#[rtse::abi]` doc).
fn abi_ty_of(ty: &syn::Type) -> Option<proc_macro2::TokenStream> {
