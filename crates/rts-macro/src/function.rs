...
pub(crate) fn expand(a: TokenStream, item: TokenStream) -> TokenStream {
    ...
    let sym = symbol_for(&args.naming, &rust_name.to_string());

    // `value = "…"` names the JS member directly; otherwise camelCase the Rust
    // fn name, same rule `#[rtse::class]`'s members use.
    // The JS name is `js_value` when given — NOT the `naming`'s value, which may
    // carry an `overload = "…"` suffix meant only for the linker symbol.
    let js_name = match (&args.js_value, &args.naming) {
        (Some(v), _) => v.clone(),
        (None, rts_abi::scope::Naming::Scoped { value: Some(v), .. }) => v.clone(),
    ...
    let (ret_abi, ret_ext_ty, ret_ts, ret_convert, ret_class_tok) =
        if let Some((cls_ty, nullable)) = &class_ret {
            wrap = true;
            let conv = if *nullable {
                ...
                quote!(::rts_engine::heap::handles::RtseReturn::__rtse_into_handle(
                    __r
                ))
            };
    ...
    // `throws` — the function reports failure through the pending-error slot, so
    // the engine must emit its post-call error check. Without this the flag was
    // pinned to NONE, and a whole class of member (every `node:assert`
    // assertion, `node:crypto`'s validating entry points, `zlib`'s sync codecs)
    // could not be declared here at all: converting one would have turned a real
    // `throw` into a silently-ignored failure. Same spelling class members use.
    let flags = if args.throws {
        quote!(::rts_engine::MemberFlags::THROWS)
    } else {
        quote!(::rts_engine::MemberFlags::NONE)
    };
    let pure_tok = args.pure;
    // `constant` — a PROPERTY read (`os.EOL`), not a call. The Registry resolves
    // it through `namespace_const`, which is a different lookup from
    // `namespace_member`, so the KIND has to say so; the symbol is still an
    // ordinary zero-arg getter. `#[rtse::constant]` covers the same ground for a
    // real Rust `const`; this is for one that must be computed.
    let kind_tok = if args.constant {
        quote!(::rts_engine::MemberKind::Constant)
    } else {
        quote!(::rts_engine::MemberKind::Function)
    };
    // An explicit `ret_ts = "…"` wins over the type-derived one — see AbiArgs.
    let ret_ts = args.ret_ts.clone().unwrap_or(ret_ts);
    // A CONSTANT is a property read, so its ts is `name: T` — no parameter list.
    // The engine parses this string to decide how a `Handle` reboxes, so the
    // shape matters, not just the prose.
    let ts_sig = if args.constant {
        format!("{js_name}: {ret_ts}")
    } else {
        format!("{js_name}({}): {ret_ts}", ts_params.join(", "))
    };
    let desc_doc = format!("ABI descriptor for `{sym}` (generated by `#[rtse::function]`).");
    let entry_doc =
        format!("Registry entry for the free function `{js_name}` — pass it to `.registry(...)`.");
