...
//! from `rts-host-rwk`'s own process.
//!
//! # Why the entry point lives here and not synthesized into the object
//!
//! `rts-host-rwk::run::run_region` already IS this sequence, written in Rust,
...
//! function-name table, install `rts-std-rwk` and `rts-node-rwk`, call the
...
//!
//! So the object built by `rts_host_rwk::object::compile_to_object` exports its
...
//! `rts_std_rwk::install`, `rts_node_rwk::install`, or a single call into
...
/// name has to match [`rts_host_rwk`]'s `object::REGION_BASE_SYMBOL` exactly —
...
unsafe extern "C" {
    ...
    /// name `rts_host_rwk::object` places every program's script under.
...
///
/// Duplicated from `rts-host-rwk::stack` rather than shared: that crate is not
/// a dependency of this one (naming it would be backwards — the facade is what
/// an AOT binary links against, and `rts-host-rwk` is a JIT host that itself
...
/// `rts-host-rwk::stack`'s own doc comment — the same absence, for the same
/// reason.
...
/// What `rts_host_rwk::object::write_manifest` wrote, read back.
///
...
/// Reads a manifest written in `rts_host_rwk::object::write_manifest`'s format.
///
...
/// `rts_host_rwk::run::compile`'s default.
...
pub extern "C" fn main(_argc: i32, _argv: *const *const i8) -> i32 {
    ...
    // `rts_host_rwk::object`'s module doc for why this exists instead of a
    ...
    rts_core_rwk::entry::declare_keys(&mut context, &manifest.keys);
    rts_core_rwk::entry::declare_literals(&mut context, &manifest.literals);
    rts_core_rwk::entry::declare_templates(&mut context, &manifest.templates);
    // No generator bodies and no stack-trace names yet — see
    // `rts_host_rwk::object`'s stated gap. An empty table each: a generator
    ...
    // at here.
    rts_core_rwk::entry::declare_frames(&mut context, Vec::new());
    rts_core_rwk::entry::declare_function_names(&mut context, Vec::new());
    ...
    rts_core_rwk::entry::declare_rest(&mut context, |wait: Duration| std::thread::sleep(wait));

    rts_std_rwk::install(&mut context);
    rts_node_rwk::install(&mut context);

    let nothing = singletons.undefined as u64;
    let (_, exit_code) = rts_core_rwk::entry::with_context(context, || {
