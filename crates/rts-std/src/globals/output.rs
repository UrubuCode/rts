...
//! stream per level; `rts-std`'s `console.rs` is that. These four are the
...
pub fn install(context: &mut Context) {
    ...
    for (name, code) in members {
        let value = rts_core::entry::make_callable(context, *code);
        rts_core::entry::declare_global(context, name, value);
    }
}

/// The text of a value, the way this crate prints one everywhere.
fn described(value: u64) -> String {
    rts_core::entry::described(value).unwrap_or_else(|| "[object]".to_owned())
}
...
extern "C" fn prompt(_e: u64, _this: u64, message: u64, _a: u64, _b: u64, _c: u64) -> u64 {
    ...
    if read == 0 {
        return rts_core::entry::null_value();
    }
