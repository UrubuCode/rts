...
mod tests {
    ...
    fn get_joins_values_with_comma_space() {
        ...
        let vh = __rtsm_global_headers_get(handle, "a".as_ptr() as i64, 1);
    ...
    fn has_and_delete() {
        let handle = h(&[("a", "1")]);
        assert!(__rtsm_global_headers_has(handle, "A".as_ptr() as i64, 1) != 0);
        __rtsm_global_headers_delete(handle, "a".as_ptr() as i64, 1);
        assert_eq!(__rtsm_global_headers_has(handle, "a".as_ptr() as i64, 1), 0);
    }

    #[test]
    fn set_replaces_all() {
        let handle = h(&[("a", "1"), ("a", "2")]);
        __rtsm_global_headers_set(handle, "a".as_ptr() as i64, 1, "9".as_ptr() as i64, 1);
    ...
    fn append_preserves_dupes() {
        let handle = h(&[]);
        __rtsm_global_headers_append(handle, "a".as_ptr() as i64, 1, "1".as_ptr() as i64, 1);
        __rtsm_global_headers_append(handle, "a".as_ptr() as i64, 1, "2".as_ptr() as i64, 1);
        with_rtse::<Headers, _>(handle, |s| {
            assert_eq!(
                s.unwrap().map.get("a"),
