...
//!
//! `persistTo` opens a file. `rts-core`'s rule 1 is availability — "present
...
///
/// # Why a side table and not an `Aside<T>` in `rts-core`
///
/// `Aside<T>` is that crate's pattern for state beside a cell it owns, and this
/// crate has no cell table of its own to key one by — `rts-core::entry`
...
/// `rts-core` has no way to mint one; `perf_hooks`, `diagnostics_channel`
...
/// The id an instance was stamped with, read outside any `rts-core` borrow.
