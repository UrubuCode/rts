...
//!
//! # What decides membership here rather than in `rts-core`
//!
...
//! **not** a second spelling of Node: `node:*` lives in `rts-node` and
...
//!
//! # `rts:egui` vive em `rts-ui`, e por quê
//!
...
//! `rts:egui` e `rts:input` são um crate próprio — `rts-ui` — atrás de uma
//! feature do host.
//!
...
pub fn install(context: &mut Context) {
    ...
    rts_core::entry::declare_module(context, "rts:test", harness);
