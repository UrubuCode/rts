...
differently from the crate this replaced (rule 2 of the README).
...
on Mach-O. This binary exports none of them, and `rts-napi` is not even
linked into it.
...
## P9 — symbol parity, and the end of the suffix (DONE)

The twenty-one names the old crate exported and this one did not: BigInt (6),
`Date` (3), instance data (2), type tags (2), `object_freeze`/`seal`,
`get_all_property_names`, `add_finalizer`, the two arraybuffer detach calls, and
the two async cleanup hooks.

**145 there, 146 here, and the diff in the direction that matters is empty.**
That is what ended the phase, and it is a claim about NAMES: eight of the 146
answer a status rather than doing the work. What it licensed was the deletion —
`crates/rts-napi` (the old engine's) is gone, this crate took its name, and the
`-rwk` suffix went with the reason it existed.

Two engine capabilities came out of it, both in `rts-core` and both usable by
anything: `bigint_from_words`/`bigint_words`/`bigint_i64`/`bigint_u64`, and
`detach_buffer`/`buffer_detached` over a `detached` mark beside the cell.

### What is left, and it is behaviour rather than names

- `napi_get_new_target` answers null, so an addon takes its non-`new` path.
- Symbols: `napi_create_symbol` cannot make one this crate can reach.
- Typed arrays: the element type and byte offset are not answered.
- Property attributes are read and ignored — `writable`, `enumerable` and
  `configurable` reach `entry/integrity.rs` for a whole object and not per key.
- An async cleanup hook does not hold teardown open; it runs and is not waited
  on. Stated in `cleanup.rs`, because the alternative is a hang.
- One addon has run. `uuid.v4` takes no arguments and keeps no state, so
  `napi_wrap`, the threadsafe path and promises are still unexercised by
  anything but this crate's own tests. Pick a harder one and run `rts napi`.
