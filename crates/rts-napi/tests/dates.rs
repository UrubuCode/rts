...
use rts_napi::{Env, dates, handles, napi_status, objects, values};
...
fn what_this_calls_a_date_is_what_the_language_calls_one() {
    in_a_program(|| {
        ...
        unsafe { rts_napi::class::napi_instanceof(raw, made, date_class, &mut instance) };
        assert!(instance);
    });
}
...
unsafe fn convert_global(
    env: rts_napi::napi_env,
    out: *mut rts_napi::napi_value,
) -> napi_status {
    let mut global = handles::none();
    // SAFETY: the caller's contract.
    unsafe { rts_napi::convert::napi_get_global(env, &mut global) };
    let name = c"Date";
