...
//! ```
//!
//! Four positional PolyValue words in `a0..a3` + a fifth `rest` PolyValue holding
//! an ARRAY (a real `Entry::Vec`, `TAG_OBJECT`) of any remaining args (or
//! `undefined` when ≤4 args). The return is one PolyValue word. A callee with ≤4
//! params reads `a0..a3` (ignoring unused); a `...rest` callee reads the overflow
//! from the `rest` array. The common ≤4-arg case never allocates `rest`. This is
//! the THUNK signature `func_addr` points at — never a variable-arity call.
...
/// through: 4 positional PolyValue words + a `rest` PolyValue (an array word or
/// `undefined`), returning one PolyValue word.
type UniformFn = extern "C" fn(u64, u64, u64, u64, u64) -> u64;

/// `__rtsadp_fn_reify(addr, nparams, has_rest)` — allocate a heap
/// [`Entry::Function`] storing the THUNK address `addr`, the declared param count
/// `nparams`, and the `has_rest` flag, and return the bare 48-bit slot+shard
/// payload (the caller boxes it as a `TAG_FUNCTION` PolyValue). Reuses
/// `Entry::Function` so NO new `Entry` variant is added.
///
...
#[unsafe(no_mangle)]
pub extern "C" fn __rtsadp_fn_reify(addr: u64, nparams: u64, has_rest: u64) -> u64 {
...
pub extern "C" fn __rtsadp_fn_invoke(
    ...
    // the bare 48-bit payload, then read the stored thunk address.
    let real = rts_runtime::namespaces::gc::handles::__RTS_FN_NS_GC_POLY_TO_HANDLE(pv.as_handle());
    let addr = with_entry(real, |e| match e {
    ...
    // the uniform `extern "C" fn(u64,u64,u64,u64,u64) -> u64` signature (see
