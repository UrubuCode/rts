...
//!  - [`REGISTRARS`] — every `fn(&mut Engine)` that pushes namespace/class metadata
...
//! (REGISTRARS) / the comment column (PRELUDE_TS) — one line, greppable.
...
pub(super) static REGISTRARS: &[Registrar] = &[
    // Namespaces backing class statics/ctors (Date.now/UTC/parse → `date`; Map/Set
    // → `collections`).
    Registrar { label: "date", run: ns::date::register, why: "backs Date statics/ctors" },
    Registrar { label: "collections", run: ns::collections::register, why: "backs Map/Set" },
    Registrar { label: "regex", run: ns::regex::register, why: "backs RegExp" },
    ...
    Registrar { label: "io", run: ns::io::register, why: "rts:io print/eprint/stdio" },
    Registrar { label: "math", run: ns::math::register, why: "rts:math + Math statics" },
    Registrar { label: "gc", run: ns::gc::register, why: "string pool / handle surface" },
    // `rts:test` framework backing namespaces, used ambiently by the embedded bundle
    // (prelude-gated in `method::try_method_dispatch`).
    Registrar { label: "test", run: ns::test::register, why: "test_core.* runner primitives" },
    Registrar { label: "globals::string", run: ns::globals::string::register, why: "string.* matchers" },
    Registrar { label: "fmt", run: ns::fmt::register, why: "fmt.parse_f64 numeric matchers" },
    // The broad std surface `tests/*.test.ts` import via `rts:<ns>`.
    Registrar { label: "fs", run: ns::fs::register, why: "rts:fs" },
    Registrar { label: "time", run: ns::time::register, why: "rts:time" },
    Registrar { label: "env", run: ns::env::register, why: "rts:env" },
    Registrar { label: "path", run: ns::path::register, why: "rts:path" },
    Registrar { label: "num", run: ns::num::register, why: "rts:num" },
    Registrar { label: "mem", run: ns::mem::register, why: "rts:mem" },
    Registrar { label: "hash", run: ns::hash::register, why: "rts:hash" },
    Registrar { label: "hint", run: ns::hint::register, why: "rts:hint" },
    Registrar { label: "ptr", run: ns::ptr::register, why: "rts:ptr" },
    Registrar { label: "buffer", run: ns::buffer::register, why: "rts:buffer" },
    Registrar { label: "alloc", run: ns::alloc::register, why: "rts:alloc" },
    Registrar { label: "bigfloat", run: ns::bigfloat::register, why: "rts:bigfloat" },
    Registrar { label: "atomic", run: ns::atomic::register, why: "rts:atomic" },
    Registrar { label: "sync", run: ns::sync::register, why: "rts:sync" },
    Registrar { label: "trace", run: ns::trace::register, why: "rts:trace" },
    Registrar { label: "process", run: ns::process::register, why: "rts:process" },
    Registrar { label: "os", run: ns::os::register, why: "rts:os" },
    Registrar { label: "crypto", run: ns::crypto::register, why: "rts:crypto" },
    Registrar { label: "net", run: ns::net::register, why: "rts:net" },
    Registrar { label: "json", run: ns::json::register, why: "rts:json namespace" },
    Registrar { label: "promise", run: ns::promise::register, why: "rts:promise" },
    Registrar { label: "parallel", run: ns::parallel::register, why: "rts:parallel" },
    Registrar { label: "thread", run: ns::thread::register, why: "rts:thread" },
    Registrar { label: "ffi", run: ns::ffi::register, why: "rts:ffi" },
    Registrar { label: "events", run: ns::events::register, why: "rts:events EventEmitter" },
    ...
    Registrar { label: "audio", run: ns::audio::register, why: "rts:audio device I/O" },
    ...
    Registrar { label: "engine", run: ns::engine::register, why: "private prelude bridges" },
    // RUNTIME/Registry global CLASS specs (ctor/methods/instanceof as metadata).
    Registrar { label: "Date class", run: ns::globals::date::register_class_spec, why: "Date class spec" },
    Registrar { label: "RegExp class", run: ns::globals::regexp::register_regexp_class_spec, why: "RegExp class spec" },
    ...
    Registrar { label: "Boolean class", run: ns::globals::boolean::register_boolean_class_spec, why: "Boolean wrapper ctor" },
    Registrar { label: "Number class", run: ns::globals::number::register_number_class_spec, why: "Number wrapper ctor" },
    Registrar { label: "String class", run: ns::globals::string::register_string_class_spec, why: "String wrapper ctor" },
];

/// One `.ts` prelude include, in DEPENDENCY ORDER (base before dependent).
/// `label`/`why` are in-table documentation (see [`Registrar`]).
#[allow(dead_code)]
