//! PEM → DER, by hand, over [`rts_core::entry::decode_base64`] — not
...
/// "permissive" stance [`rts_core::entry::decode_base64`] itself
...
pub(crate) fn blocks(pem: &str) -> Vec<Vec<u8>> {
    ...
    for line in pem.lines() {
        ...
        if trimmed.starts_with("-----END ") {
            if inside {
                found.push(rts_core::entry::decode_base64(&body));
            }
            inside = false;
            continue;
        }
