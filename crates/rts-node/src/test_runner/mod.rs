...
//! # Reuse-check
//!
//! `rts-cranelift` owns nothing test-shaped. `crates/rts-std/src/test.rs`
...
//! `rts-std::test`'s own `describe`/`test` already use — because nothing
...
//! region to unwind into (`crate::assert`, `rts-std::test`'s own module
...
/// host reads, the same shape `rts-std::test::record` has for its own
/// (separate) record.
