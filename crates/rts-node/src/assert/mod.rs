...
//! [`rts_core::entry::throw`]'s own module doc has the history: finding a
...
//! and it is fixed: `rts_core::entry::throw_type_error(message)` is
...
//! Node's `name`/`code`/`actual`/`expected` shape. `rts-std`'s
...
//!
//! - `rts_core::entry` (read in full): `strict_equals` is `===` and
