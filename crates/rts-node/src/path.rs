...
pub fn namespace(context: &mut Context) -> u64 {
    ...
    let separator = rts_core::entry::make_string(context, "/");
    rts_core::entry::put_member(context, namespace, "sep", separator);
    let delimiter = rts_core::entry::make_string(context, ":");
    rts_core::entry::put_member(context, namespace, "delimiter", delimiter);
    rts_core::entry::put_member(context, namespace, "posix", posix);
    rts_core::entry::put_member(context, namespace, "win32", win32);
    rts_core::entry::put_member(context, posix, "posix", posix);
    rts_core::entry::put_member(context, posix, "win32", win32);
    rts_core::entry::put_member(context, win32, "posix", posix);
    rts_core::entry::put_member(context, win32, "win32", win32);
    namespace
}
...
fn flavor_namespace(context: &mut Context, sep: char) -> u64 {
    ...
    let namespace = rts_core::entry::make_namespace(context, members);
    let separator = rts_core::entry::make_string(context, &sep.to_string());
    rts_core::entry::put_member(context, namespace, "sep", separator);
    let delimiter_text = if sep == '\\' { ";" } else { ":" };
    let delimiter = rts_core::entry::make_string(context, delimiter_text);
    rts_core::entry::put_member(context, namespace, "delimiter", delimiter);
    namespace
}
...
/// vector `String.fromCharCode` and `Math.max` do, the way `rts-core`'s
...
fn parse_g(value: u64, sep: char) -> u64 {
    ...
    rts_core::entry::with_runtime(|context| {
        ...
        for (key, value) in [
            ...
            let held = rts_core::entry::make_string(context, value);
            rts_core::entry::put_member(context, object, key, held);
        }
        object
    })
}
...
fn format_g(obj: u64, sep: char) -> u64 {
    ...
    let (dir_v, root_v, base_v, ext_v, name_v) = rts_core::entry::with_runtime(|context| {
        (
            rts_core::entry::get_member(context, obj, "dir"),
            rts_core::entry::get_member(context, obj, "root"),
            rts_core::entry::get_member(context, obj, "base"),
            rts_core::entry::get_member(context, obj, "ext"),
            rts_core::entry::get_member(context, obj, "name"),
        )
    });
...
extern "C" fn matches_glob_entry(
    ...
    let path = rts_core::entry::text_of(path).unwrap_or_default();
    let pattern = rts_core::entry::text_of(pattern).unwrap_or_default();
    rts_core::entry::boolean_value(matches_glob(&path, &pattern))
}
...
///
/// `rts_core::entry::arguments_at` is the one place that reads the
