...
//! `rts-core/src/entry/modules.rs`, the whole host surface, for a numeric,
...
pub fn namespace(context: &mut Context) -> u64 {
    ...
    let eol = rts_core::entry::make_string(context, EOL);
    rts_core::entry::put_member(context, namespace, "EOL", eol);
    let dev_null = rts_core::entry::make_string(context, DEV_NULL);
    rts_core::entry::put_member(context, namespace, "devNull", dev_null);
    let table = constants::object(context);
    rts_core::entry::put_member(context, namespace, "constants", table);
    namespace
}
...
extern "C" fn loadavg(_e: u64, _this: u64, _a0: u64, _a1: u64, _a2: u64, _a3: u64) -> u64 {
    ...
    rts_core::entry::with_runtime(|context| {
        let values = held.iter().map(|held| rts_core::entry::make_number(*held)).collect();
...
extern "C" fn network_interfaces(_e: u64, _this: u64, _a0: u64, _a1: u64, _a2: u64, _a3: u64) -> u64 {
    ...
    rts_core::entry::with_runtime(|context| netif::value(context, held))
}
...
extern "C" fn user_info(_e: u64, _this: u64, options: u64, _a1: u64, _a2: u64, _a3: u64) -> u64 {
    rts_core::entry::with_runtime(|context| {
        ...
        let encoding = rts_core::entry::get_member(context, options, "encoding");
        let wants_buffer = rts_core::entry::string_in(context, encoding)
...
extern "C" fn get_priority(_e: u64, _this: u64, pid: u64, _a1: u64, _a2: u64, _a3: u64) -> u64 {
    let pid = rts_core::entry::number_of(pid).unwrap_or(0.0).max(0.0) as u32;
    match user::priority_of(pid) {
        Some(held) => rts_core::entry::make_number(held),
...
extern "C" fn set_priority(_e: u64, _this: u64, first: u64, second: u64, _a2: u64, _a3: u64) -> u64 {
    ...
    let (pid, priority) = match rts_core::entry::number_of(second) {
        Some(priority) => (rts_core::entry::number_of(first).unwrap_or(0.0), priority),
        None => (0.0, rts_core::entry::number_of(first).unwrap_or(0.0)),
    };
    rts_core::entry::boolean_value(user::set_priority(pid.max(0.0) as u32, priority))
}
