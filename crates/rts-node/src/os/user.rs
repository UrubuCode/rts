...
pub(super) fn value(context: &mut rts_core::entry::Context, as_buffer: bool) -> u64 {
    let Some(held) = current() else {
        return rts_core::entry::undefined_in(context);
    };
    let object = rts_core::entry::make_object(context);
    let text = |context: &mut rts_core::entry::Context, held: &str| match as_buffer {
        true => rts_core::entry::make_buffer(context, held.as_bytes()),
        false => rts_core::entry::make_string(context, held),
    };
    let username = text(context, &held.username);
    rts_core::entry::put_member(context, object, "username", username);
    let homedir = text(context, &held.homedir);
    rts_core::entry::put_member(context, object, "homedir", homedir);
    let shell = match &held.shell {
        Some(found) => text(context, found),
        None => rts_core::entry::null_in(context),
    };
    rts_core::entry::put_member(context, object, "shell", shell);
