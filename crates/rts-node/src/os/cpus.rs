...
/// reason.
pub(super) fn value(context: &mut rts_core::entry::Context, held: Vec<Cpu>) -> u64 {
    let entries = held
        .into_iter()
        .map(|cpu| {
            let object = rts_core::entry::make_object(context);
            let model = rts_core::entry::make_string(context, &cpu.model);
            rts_core::entry::put_member(context, object, "model", model);
            let speed = rts_core::entry::make_number(cpu.speed);
            rts_core::entry::put_member(context, object, "speed", speed);
            let times = rts_core::entry::make_object(context);
            for (name, value) in [
                ("user", cpu.user),
                ("nice", cpu.nice),
                ("sys", cpu.sys),
                ...
                let number = rts_core::entry::make_number(value);
                rts_core::entry::put_member(context, times, name, number);
            }
            rts_core::entry::put_member(context, object, "times", times);
            object
        })
        .collect();
    rts_core::entry::make_array_in(context, entries)
}
