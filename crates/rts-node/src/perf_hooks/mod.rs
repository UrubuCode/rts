...
//! `rts-core::entry::Context` (read in `entry/mod.rs`) holds no timing
//! field.
//!
//! Inside this crate the search found the pieces this module now reuses rather
//! than repeats: [`rts_core::entry::make_prototype`]'s name-keyed identity
//! (the same tool `async_hooks::attach` uses to give a class ONE prototype),
//! [`rts_core::entry::declare_loop_source`] (the one answer to "a module
//! with work left after the last statement", written because six modules had
//! six copies of it), and [`rts_core::entry::closure_new`] — which is what
...
//!
//! `rts-std/src/globals/timing.rs` builds a `performance` object too, and
...
//! [`rts_core::entry::make_prototype`] under the name `"Performance"`,
...
//!
//! The residue that cannot be removed from here: `rts-std`'s `ORIGIN`
...
pub fn namespace(context: &mut Context) -> u64 {
    ...
    // whether this call or `rts-std`'s was the first.
