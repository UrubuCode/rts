...
//! (`rts_core::entry::current`'s own doc: "aborts when there is none").
...
//! `rts-node` that holds a JS value across more than one native call (see
...
//! (`rts_core::heap`'s handle table is the abstraction that would absorb
