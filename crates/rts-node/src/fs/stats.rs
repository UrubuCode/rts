...
//! objects alongside their `*Ms` millisecond twins. `rts_core::entry`
//! exposes no constructor a host can call to build a `Date` value — the class
//! exists (`crates/rts-core/src/entry/date/mod.rs`) but nothing in the
...
/// `rts-core`'s README — see that function's doc for why the class is
...
/// outside `rts-core` (only `throw_type_error` itself is public), so a
