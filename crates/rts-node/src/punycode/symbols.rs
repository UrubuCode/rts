//! node:punycode — base extern "C" symbol implementations (the sync surface).
//!
//! Punycode (RFC 3492) + IDNA ToASCII/ToUnicode. Native rts-node implementation
//! (no rts-std mirror). Pure `string`→`string`; symbols use the
//! `__RTS_FN_NODE_PUNYCODE_*` convention. The module is deprecated in Node but
//! still shipped, so it is part of the parity surface.
//!
