...
//! at the end, inside a single [`with_runtime`](rts_core::entry::with_runtime)
//! call. `stringify`/`encode` are the one direction that has to read an
//! arbitrary program value apart: it walks
//! [`own_keys`](rts_core::entry::own_keys) and reads each value with
//! [`described`](rts_core::entry::described), which is this crate's
...
//! [`rts_core::entry::rest_arguments`] needed. `options.maxKeys` truncates
...
//! codec for that one call, invoked through [`rts_core::entry::call`].
//! Rule 8 of `rts-core`'s README asks a native calling user code to check
...
extern "C" fn parse(_e: u64, _this: u64, text: u64, sep: u64, eq: u64, options: u64) -> u64 {
    ...
    rts_core::entry::with_runtime(|context| {
        let result = rts_core::entry::make_object(context);
        for (key, values) in grouped {
            let value = match values.len() {
                1 => rts_core::entry::make_string(context, &values[0]),
                _ => {
                    let strings = values.iter().map(|v| rts_core::entry::make_string(context, v)).collect();
                    rts_core::entry::make_array_in(context, strings)
                }
            };
            rts_core::entry::put_member(context, result, &key, value);
        }
        result
    })
}
...
extern "C" fn stringify(_e: u64, _this: u64, object: u64, sep: u64, eq: u64, options: u64) -> u64 {
    ...
    if object == absent {
        return rts_core::entry::with_runtime(|context| rts_core::entry::make_string(context, ""));
    }
    let keys = collect_array(rts_core::entry::own_keys(object));
    let mut pairs = Vec::new();
    for key in keys {
        let Some(name) = rts_core::entry::text_of(key) else {
            continue;
        };
        let value = rts_core::entry::get_indexed(object, key);
    ...
    rts_core::entry::with_runtime(|context| rts_core::entry::make_string(context, &joined))
}
...
fn option_callback(options: u64, name: &str) -> Option<u64> {
    ...
    let value = rts_core::entry::get_indexed(options, rts_core::entry::with_runtime(|context| rts_core::entry::make_string(context, name)));
    let callable = rts_core::entry::with_runtime(|context| rts_core::entry::is_callable_in(context, value));
...
fn option_max_keys(options: u64) -> Option<usize> {
    ...
    let value = rts_core::entry::get_indexed(options, rts_core::entry::with_runtime(|context| rts_core::entry::make_string(context, "maxKeys")));
    let count = rts_core::entry::number_of(value)?;
...
fn call_codec(callback: u64, text: &str) -> String {
    let absent = rts_core::entry::undefined_value();
    let argument = rts_core::entry::with_runtime(|context| rts_core::entry::make_string(context, text));
    let result = rts_core::entry::call(callback, absent, argument, absent, absent, absent);
    rts_core::entry::text_of(result).unwrap_or_default()
}
...
///
/// A scalar goes through [`described`](rts_core::entry::described) — this
...
fn stringify_value(value: u64) -> Vec<String> {
    if let Some(text) = rts_core::entry::described(value) {
        return vec![text];
    }
    ...
    elements
        .into_iter()
        .map(|element| rts_core::entry::described(element).unwrap_or_default())
        .collect()
}
...
fn collect_array(array: u64) -> Vec<u64> {
    ...
    let length_key = rts_core::entry::with_runtime(|context| rts_core::entry::make_string(context, "length"));
    let length_value = rts_core::entry::get_indexed(array, length_key);
