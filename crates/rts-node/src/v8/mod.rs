...
//!
//! Inside `rts-core`: [`rts_core::entry::Context`] holds `region`
...
//! from neither `rts_core::entry::modules` nor here; the second needs a
...
fn region_bytes(context: &Context) -> (f64, f64) {
    let stride = u64::from(rts_core::heap::STRIDE);
...
extern "C" fn get_heap_statistics(_e: u64, _this: u64, _a: u64, _b: u64, _c: u64, _d: u64) -> u64 {
    rts_core::entry::with_runtime(|context| {
        ...
        for (name, value) in fields {
            let held = rts_core::entry::make_number(value);
            rts_core::entry::put_member(context, object, name, held);
        }
        object
    })
}
...
extern "C" fn get_heap_space_statistics(
    ...
    rts_core::entry::with_runtime(|context| {
        let (total, used) = region_bytes(context);
        let space = rts_core::entry::make_object(context);
        let name = rts_core::entry::make_string(context, "rts_region");
        rts_core::entry::put_member(context, space, "space_name", name);
        let fields = [
            ("space_size", total),
        ...
        for (field, value) in fields {
            let held = rts_core::entry::make_number(value);
            rts_core::entry::put_member(context, space, field, held);
        }
        rts_core::entry::make_array_in(context, vec![space])
    })
}
...
extern "C" fn cached_data_version_tag(
    ...
    rts_core::entry::make_number(f64::from(TAG))
}
...
extern "C" fn is_string_one_byte_representation(
    ...
    rts_core::entry::with_runtime(|context| {
        match rts_core::entry::string_in(context, content) {
            Some(text) => rts_core::entry::boolean_value(text.is_ascii()),
            None => rts_core::entry::undefined_in(context),
        }
    })
}
