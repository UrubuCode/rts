...
//!   [`rts_core::entry::declare_loop_source`] / `Pending`, already the ONE
...
//!   again.
//! - *"a bigint from Rust"* — [`rts_core::entry::make_bigint`], which
...
/// it under [`rts_core::entry::with_runtime`] is the nested borrow that
...
fn emit(event: &str, argument: u64) -> bool {
    ...
    let (emitter, name) = rts_core::entry::with_runtime(|context| {
        (
            rts_core::entry::get_member(context, process, "emit"),
            rts_core::entry::make_string(context, event),
        )
    });
    ...
    let answered = rts_core::entry::call(emitter, process, name, argument, absent, absent);
    rts_core::entry::to_boolean(answered)
}
