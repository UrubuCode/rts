...
//! entirely value encodings and ABI. `rts-std::console` (`crates/rts-std/
...
//! `rts_core::entry::described`, which is `None` for an object — "an
...
//! `"inspect"` member through [`rts_core::entry::call`] — one struct walk,
...
//! and lives in `rts-std`, unmodified by this module. This crate's stream
...
/// `rts-std::console`'s own `line` helper (same reasoning, restated
...
/// `rts-std::console`'s destination for the common case.
