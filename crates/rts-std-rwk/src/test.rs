...
//! machine's protected regions on the call path, which `rts-host-rwk`'s plan
...
//! beside a cell is `rts-core-rwk`'s mechanism and is not exported, and a test
...
thread_local! {
    ...
    ///
    /// Thread-local because a context is: `rts-core-rwk` holds one per thread
...
extern "C" fn describe(_e: u64, _this: u64, _name: u64, body: u64, _a2: u64, _a3: u64) -> u64 {
    ...
    rts_core_rwk::entry::call(body, absent, absent, absent, absent, absent)
}
...
extern "C" fn test(_e: u64, _this: u64, name: u64, body: u64, _a2: u64, _a3: u64) -> u64 {
    let title = rts_core_rwk::entry::described(name).unwrap_or_default();
    ...
    let absent = rts_core_rwk::entry::undefined_value();
    let answered = rts_core_rwk::entry::call(body, absent, absent, absent, absent, absent);
...
extern "C" fn expect(_e: u64, _this: u64, value: u64, _a1: u64, _a2: u64, _a3: u64) -> u64 {
    ...
    rts_core_rwk::entry::with_runtime(|context| {
        rts_core_rwk::entry::put_member(context, object, RECEIVED, value);
        for (name, code) in members {
            let method = rts_core_rwk::entry::make_callable(context, *code);
            rts_core_rwk::entry::put_member(context, object, name, method);
        }
    });
    object
}
...
extern "C" fn to_be_truthy(_e: u64, this: u64, _a0: u64, _a1: u64, _a2: u64, _a3: u64) -> u64 {
    let received = received_of(this);
    let held = rts_core_rwk::entry::to_boolean(received);
...
fn received_of(this: u64) -> u64 {
    rts_core_rwk::entry::with_runtime(|context| {
        rts_core_rwk::entry::get_member(context, this, RECEIVED)
    })
}
...
fn settle(held: bool, received: u64, expected: u64) -> u64 {
    if !held {
        let described = format!(
            "expected {}, received {}",
            rts_core_rwk::entry::described(expected).unwrap_or_else(|| "an object".to_owned()),
            rts_core_rwk::entry::described(received).unwrap_or_else(|| "an object".to_owned())
        );
