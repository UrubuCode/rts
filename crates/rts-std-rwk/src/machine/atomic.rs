...
//!
//! # Reuse-check: this is not [`rts_core_rwk::entry::buffers::atomics`]
//!
...
//!
//! One OS thread runs JavaScript in this engine today — `crates/rts-host-rwk`
//! has no code that starts a second one to run a callback, and
//! `crates/rts-core-rwk`'s `Context` is reached through a thread-local
...
//! only at the two sites `crates/rts-core-rwk/README.md` names, and this is not
//! one of them.
