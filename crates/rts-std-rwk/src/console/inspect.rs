...
//! `rts-core-rwk` (rule 2 of that crate's README: ask the machine — here, the
...
//! the callable — a gap in `rts-core-rwk`, not this file — so every function
...
/// `get_property`, `forEach` through `call` — per rule 8 of `rts-core-rwk`'s
...
///
/// `rts_core_rwk::Value` is exported from the crate root for exactly this: a
...
fn as_slot(value: u64) -> Option<u32> {
    rts_core_rwk::Value(value).as_slot()
}
...
/// reading `rts-core-rwk`'s collection table directly, which is private to that
...
/// stack out as text (`rts-core-rwk`'s `throw::stack_text`, read back here
