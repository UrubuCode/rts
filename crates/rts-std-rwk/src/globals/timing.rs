...
//! a benchmark stopwatch, not a service. `rts-core-rwk`'s `entry::Context`
...
//!   `rts-core-rwk`'s `entry/global.rs` answers the name with the global object
...
//!   `rts-core-rwk`'s `entry/clone.rs` is the walk *and* the global: its
...
//!
//! `crates/rts-node-rwk/src/perf_hooks.rs` builds a `performance` object with
//! its own `OnceLock<Instant>` origin, and `rts-node-rwk`'s `install` already
//! declares it as a global (its `from_modules` list names
//! `("perf_hooks", &["performance"])`). So after
//! `crates/rts-host-rwk/src/run.rs` runs `rts_std_rwk::install` and then
//! `rts_node_rwk::install`, **node's object wins the name** — which is the
...
//! origins, and this crate cannot reach across to fix it: `rts-std-rwk`'s
//! manifest depends on `rts-core-rwk` alone, and depending on `rts-node-rwk`
...
//! object is minted through [`rts_core_rwk::entry::make_prototype`], which is
...
//! is [`rts_core_rwk::entry::settled`], which hands back a promise that is
