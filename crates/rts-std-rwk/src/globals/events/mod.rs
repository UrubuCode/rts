...
//! - **`rts-core-rwk::entry`** (`modules.rs`, read in full) — `make_prototype`,
...
//! - **`rts-node-rwk/src/events.rs`** — `EventEmitter`, read in full. Its
...
//! what `rts-host-rwk`'s rule 3 asks for: `rts-core-rwk::entry::loops` exists
//! precisely so a module owns its own deadline table and registers a `fn`
//! pointer the host pumps — six modules in `rts-node-rwk` do it, `node:timers`
//! among them. Using that contract is reuse; depending on `node:timers` would
//! have been the duplication.
//!
//! A pending timeout answers [`Pending::Blocked`](rts_core_rwk::entry::Pending),
...
//! building a class from outside `rts-core-rwk`, where `#[rtse::class]` and its
