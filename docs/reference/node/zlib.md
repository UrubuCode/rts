...
as the rest of RTS's async surface (`the spec removed 2026-08-03 (see git history)`):
...
- **Stream/codec handles are thread-owned, not shared.** A `Deflate`/
  ...
  (`the spec removed 2026-08-03 (see git history)`), these handles live in a
...
- **Threadpool contention is the only cross-thread concern**, and it is
  ...
  sizing (`the spec removed 2026-08-03 (see git history)` / `02-runtime.md`) — `node:zlib`
