...
- **Promise/settle subsystem — weaker dependency than in most async node modules.** Because `node:tls`'s own API surface is entirely event/callback-based (no `tls.promises`), it does not need the engine's Promise-create/settle machinery (`the spec removed 2026-08-03 (see git history)`) as directly as e.g. `node:dns`'s promise-mirrored surface does. It is still worth confirming whether the underlying host wake-up mechanism that drains the `.ts` event-loop's poll queue each turn shares any machinery with the Promise subsystem's microtask draining — flagged as a "verify degree of overlap" item rather than a hard blocker.
