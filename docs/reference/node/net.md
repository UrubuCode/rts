...
- **Promise/async settle subsystem.** Needed only for `server[Symbol.asyncDispose]()` in this module's happy path (every other async surface here is callback/event-driven, not Promise-driven) — but every other async `node:*` module needs it too (per `the spec removed 2026-08-03 (see git history)`), so this remains a hard cross-cutting prerequisite currently living in `rts-std`'s `promise` namespace; must be hoisted into `rts-engine` or a new shared low crate before `rts-node` can implement `Symbol.asyncDispose` without depending on `rts-std`.
