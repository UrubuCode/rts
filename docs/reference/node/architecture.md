...
RTS has its own threading model (`the spec removed 2026-08-03 (see git history)`): per-thread
...
1. **Fate of the `rts:*` OS-ish surface.** `rts:fs`/`rts:os`/`rts:net`/… are
   defined by the approved `docs/engine/architecture.md`. Moving the bodies to
