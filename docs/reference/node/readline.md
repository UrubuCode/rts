...
- An `Interface`/`Readline` instance is ordinary **per-thread-region heap data** under `the spec removed 2026-08-03 (see git history)` — `threadLocal` by construction, exactly like `node:events`' `EventEmitter` (per `events.md` §5.4). Nothing in this module's public surface needs `shared`/promotion-on-publication semantics.
