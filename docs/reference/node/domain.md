...
- The active-domain stack is thread-local `.ts` module state (a gcell), which
  ...
  `Worker` (mapped to an RTS thread/region per `the spec removed 2026-08-03 (see git history)`)
