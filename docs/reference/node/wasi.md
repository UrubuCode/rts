...
- **`finalizeBindings(instance[, { memory }])` is this module's entire
  ...
  onto RTS's threading model (`the spec removed 2026-08-03 (see git history)`) as
...
- **Where the `WebAssembly` global itself is designed and owned** — a new
  ...
  doc, analogous to how `the spec removed 2026-08-03 (see git history)` got its own doc
