...
- **The global marks/measures/entries buffers, and the set of active
  ...
  (tokio blocking-pool workers, per `the spec removed 2026-08-03 (see git history)`),
...
- **Cross-worker histogram cloning**: when a `worker_threads` `MessagePort`
  ...
  (`the spec removed 2026-08-03 (see git history)`): the clone promotes only the
...
- **Per-worker vs process-wide global timeline** (§5.4): this spec proposes a
  ...
  mapping (`the spec removed 2026-08-03 (see git history)`) argues for a different
