...
- **`AsyncLocalStorage.run`/`exit`/`enterWith`/`withScope`**: sync at the call
  ...
  - **`async function` / `await` / `Promise`**: per `the spec removed 2026-08-03 (see git history)`,
...
- Because `promise.create` genuinely hops OS threads (§5.3), the context stack
  ...
  model (`the spec removed 2026-08-03 (see git history)`): a store handle captured on
...
- **`worker_threads` interaction** is written here on the assumption of "fresh
  ...
  region lifecycle mapping (`the spec removed 2026-08-03 (see git history)`) may add
