...
- **Distinct from RTS's own existing `test`/`bundle.ts` harness.** The RTS
  ...
  `bundle.ts`, described/used throughout the project's own `CLAUDE.md` +
...
- **Default `isolation: 'process'`** spawns a genuinely separate **OS
  ...
  `worker_threads`/threading-model surface (`the spec removed 2026-08-03 (see git history)`):
...
- **promise settle** — needed for callback-style test functions (`(t, done)
  ...
  (`the spec removed 2026-08-03 (see git history)`) primarily describes the
