...
- `TracingChannel.tracePromise`: composes with the **existing** RTS Promise surface
  ...
  design — see `CLAUDE.md`) purely from the `.ts` shim, i.e. it is
...
(`the spec removed 2026-08-03 (see git history)`), the channel registry is **per-realm, not
...
- **`node:async_hooks` / `AsyncLocalStorage` — does not exist anywhere in RTS yet.**
  ...
  model, which per `CLAUDE.md` is currently *interim synchronous*).
...
- **`WeakReference`/`FinalizationRegistry`-based automatic channel GC** is a pure memory
  ...
  currently strong-ref interim (per `CLAUDE.md` — real weak semantics land with the
