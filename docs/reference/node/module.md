...
- **Promise/async settle subsystem** (`rts-std`'s `promise` namespace, per `the spec removed 2026-08-03 (see git history)`) — needed for `module.register()`'s returned promise and (pending the §2 ambiguity) `module.flushCompileCache()`. Must be reachable from `rts-node` without an `rts-std` dependency.
