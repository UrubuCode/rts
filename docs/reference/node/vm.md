...
- **The compile-and-run primitive already exists, narrower than `vm` needs.**
  ...
  `the spec removed 2026-08-03 (see git history)` and `CLAUDE.md`,
...
- **What genuinely does not exist yet: a "compile bound to an alternate
  ...
  1. **(Recommended direction) A scope/region handle threaded through the
     ...
     model already establishes (`the spec removed 2026-08-03 (see git history)`,
...
IR the *codegen* emits at each call site (`docs/engine/architecture.md`
...
| `__RTS_FN_NODE_VM_SCRIPT_COMPILE` | `StrPtr` (code), `StrPtr` (filename), `I32` (line_offset), `I32` (column_offset) | `Handle` (script) | Parses/compiles `code` without running it. Throws-equivalent: a compile failure sets a thread-local error slot the `.ts` layer checks (existing engine error-slot convention, `the spec removed 2026-08-03 (see git history)`). |
...
- **Context/script/module `Handle`s must be safely referenceable from any OS
  ...
  `promise.create`-spawned callbacks, per `the spec removed 2026-08-03 (see git history)`)
