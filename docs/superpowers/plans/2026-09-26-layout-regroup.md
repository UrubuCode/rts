...
`layout/` is 76 files and 18 678 lines, all FLAT beside `layout.rs` — the only
...
| `dimensao_indefinida.rs` | `flex/indefinite_size.rs` | flex (grid reads it as `super::super::flex::`) |
...
| `fonte_avancos.rs` | `measure/font_advances.rs` | measure (GENERATED table; its generator's output path changes too) |
...
Seventy-five rows; `layout.rs` and `tests/` stay. If an agent finds a file on
...
- 75 flat files + `layout.rs` (487 lines) + `tests/` (97 files, `mod.rs` 360
...
- `fonte_avancos.rs` is GENERATED (its header says by what); the generator's
  output path must follow the file or the next regeneration recreates the old
  path. Find the generator by grepping the file's header line.
...
- [ ] **R1 — `measure/` and `fragment/`.** The two leaf folders. Create
  ...
  the generator's output path for `font_advances.rs`. Check.
...
- [ ] **R5 — docs and the PLAN row** (coordinator, after the rulers):
  `docs/ui/html-engine/box-tree.md` §6 names `layout/` files — update the
  paths; `crates/rts-dom/README.md` if it lists files; PLAN.md §0 gains a
