...
| Texto guardado | `font-stretch`, `word-spacing`, `text-overflow`, `text-wrap:balance/pretty`, `tab-size`, `line-clamp`, `line-break`, `direction:rtl`, `unicode-bidi`, `hyphens`, `list-style-type` e `list-style-image`. | `direction` não inverte o fluxo, bidi não existe, a hifenização automática (dicionário) não existe — a manual com `&shy;` está feita (lote S-hifen) —, marcadores não são desenhados; algumas opções de quebra e overflow não chegam ao medidor. |
