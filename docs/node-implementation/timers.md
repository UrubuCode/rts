...
- **Event loop epilogue** (`event_loop.rs::run_event_loop`, exposed as the extern `__RTS_FN_RT_RUN_EVENT_LOOP`): drains, in JS-spec order, microtasks → immediates → macrotasks (which themselves drain their own microtasks per callback) → pending one-shot timers → fire-and-forget promises → microtasks again → unhandled-rejection reporting. Called host-side after `__RTS_MAIN` on the JIT path, and from the AOT shim `main` before it returns (this was the fix that made `await`/`.then`/`setTimeout` work in `rts compile` binaries at all).
...
- **AOT vs JIT**: identical — both call the same `__RTS_FN_RT_RUN_EVENT_LOOP` epilogue (JIT: host-side after `__RTS_MAIN`; AOT: from the generated `main` shim before it returns), so a `node:timers` timeout/interval/immediate registered during either compiled form's execution is drained the same way.
