...
> **Estado na branch de cutover (`feat/rts-codegen-new`, 2026-06-19):** o crate
> `crates/rts-napi/` (159 fns N-API) foi PORTADO para a branch do motor novo — a
> camada de engine (`rts-engine`: `Entry::ArrayBuffer`/`BigInt`/`NapiExternal` +
> ptr estável + fila de finalizers `drain_pending_napi_finalizers`) está presente,
> o crate compila (59 testes de lib verdes) e `rts-runtime` re-exporta via
> `pub use rts_napi as napi`. **PENDENTE no motor novo:** o LOWERING de
> `require('x.node')` → `__RTS_FN_NS_NAPI_LOAD_ADDON` (o motor novo ainda não tem
> `require`/dynamic-import) + reter os símbolos `napi_*` no export-table do bin
> (`force_link`) p/ o `dlopen` do addon resolver. O texto abaixo descreve a
> implementação completa (originada no motor antigo, hoje deletado).
