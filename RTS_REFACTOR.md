...
- [x] `crates/rts-codegen/src/mir_codegen/`: lower MIR → Cranelift IR isolado (não plugado no pipeline). hint_bridge.rs converte CraneliftTypeHint → cl::Type; lower.rs traduz Inst/Terminator 1:1 via FunctionBuilder. **22 testes** cobrindo: (a) 10 unitários JIT-execute que constroem MirFunc na mão (incluindo Switch e loop com phi nodes); (b) 7 de integração via `rts_parser::parse_source` + lower HIR + lower MIR + passes + verify; (c) **5 smoke end-to-end TS source → MIR → Cranelift JIT → execução nativa**, provando que `inc/max/poly/mask/abs` retornam valores corretos quando compilados pelo caminho MIR.
